'use strict';
/* AI HEALTH v33 — APP (вся логика, финал) */
...
var OLD_KEYS=['ai_health_v32','ai_health_v31','ai_health_v30'];
...
function defaultState(){
  return{
    ...
    detoxCourseProgress:{},
    challengeProgress:{},dailySurveys:{},todayPlan:null,surveys:{},
...
function startEffects(){
  ...
  try{
    ...
    var theme=(window.THEMES||[]).find(function(t){return t.id===state.settings.theme});
...
function navigate(page){
  ...
  var renderers={
    ...
    survey:function(){if(typeof renderSurveyStep==='function'&&window.currentSurveyId)renderSurveyStep()}
  };
...
function openLiveControl(){
  ...
  var items=[
    {icon:'📋',label:'Опрос дня',action:'if(typeof openSurvey===\'function\'&&typeof getPendingSurveys===\'function\'){var p=getPendingSurveys();if(p.length)openSurvey(p[0].id);else toast(\'Все опросы сделаны\',\'info\')}closeLiveControl()'},
...
function buildTodayPlan(answers){
  ...
  if(sleep<6)plan.notes.push('⚠️ Недосып.');
  if(screen>360)plan.notes.push('⚠️ Много экрана.');
  if(stress>7)plan.notes.push('⚠️ Стресс.');
  return plan;
}
...
function showWelcome(){
  ...
  overlay.innerHTML='<div class="welcome-logo">🧠</div><div class="welcome-title">AI Health v33</div><div class="welcome-sub">One UI 9 · 5555+ единиц контента · Опросы · Адаптация · AI.</div><button class="btn btn-primary btn-block" style="max-width:340px;" onclick="startOnboarding()">Начать</button>';
  document.body.appendChild(overlay);
}
