...
function gcalConnect(clientId){
  ...
  if(!g.clientId){toastInt('Укажи Client ID','error');return false}
  g.connected=true;
  g.lastSync=null;
  saveInt();
...
function gcalDisconnect(){
  var g=getInt('googleCalendar');
  g.connected=false;g.accessToken=null;g.lastSync=null;
  saveInt();toastInt('Google Calendar отключён','info');
}
function gcalSync(){
  var g=getInt('googleCalendar');
  if(!g.connected){toastInt('Сначала подключи','warning');return false}
  g.lastSync=new Date().toISOString();
  saveInt();toastInt('Синхронизация... (demo)','success');
  return true;
}
function gcalIsConnected(){return !!getInt('googleCalendar').connected}
...
function obsidianConnect(apiKey,folder){
  ...
  if(!o.apiKey){toastInt('Укажи API Key','error');return false}
  saveInt();toastInt('Obsidian подключён','success');
  return true;
}
function obsidianDisconnect(){
  var o=getInt('obsidian');
  o.apiKey='';o.lastSync=null;
  saveInt();toastInt('Obsidian отключён','info');
}
function obsidianExport(){
  var o=getInt('obsidian');
  if(!o.apiKey){toastInt('Сначала подключи','warning');return false}
  o.lastSync=new Date().toISOString();
  saveInt();toastInt('Экспорт в Obsidian (demo)','success');
  return true;
}
function obsidianIsConnected(){return !!getInt('obsidian').apiKey}
...
function notionConnect(apiKey,databaseId){
  ...
  if(!n.enabled){toastInt('Заполни API Key и Database ID','error');return false}
  saveInt();toastInt('Notion подключён','success');
  return true;
}
function notionDisconnect(){
  var n=getInt('notion');
  n.apiKey='';n.databaseId='';n.enabled=false;
  saveInt();toastInt('Notion отключён','info');
}
function notionIsConnected(){return !!getInt('notion').enabled}
...
function todoistConnect(apiKey){
  ...
  t.enabled=!!t.apiKey;
  if(!t.enabled){toastInt('Укажи API Key','error');return false}
  saveInt();toastInt('Todoist подключён','success');
  return true;
}
function todoistDisconnect(){
  var t=getInt('todoist');
  t.apiKey='';t.enabled=false;
  saveInt();toastInt('Todoist отключён','info');
}
function todoistIsConnected(){return !!getInt('todoist').enabled}
function todoistExportTasks(){
  var t=getInt('todoist');
  if(!t.enabled){toastInt('Сначала подключи','warning');return false}
  var tasks=[];
  try{var s=getState();tasks=(s.tasks||[]).filter(function(x){return x.status!=='completed'})}catch(e){}
...
function geminiDisconnect(){
  var g=getInt('gemini');
  g.apiKey='';g.connected=false;
  var s=getState();
  if(s&&s.settings)s.settings.apiKey='';
  saveInt();toastInt('Gemini отключён','info');
}
function geminiIsConnected(){return !!getInt('gemini').apiKey}
async function geminiChat(prompt,systemPrompt){
  var g=getInt('gemini');
  if(!g.apiKey){toastInt('Gemini не подключён','warning');return null}
  try{
  ...
  }catch(e){toastInt('Ошибка Gemini','error');return null}
}
