...
function getEntertainmentStats(){
  var watched=(window.state&&window.state.watched)||[];
  var watchlist=(window.state&&window.state.watchlist)||[];
  var stats={
    movies_watched:watched.filter(function(w){return w.type==='movie'}).length,
    series_watched:watched.filter(function(w){return w.type==='series'}).length,
    books_read:watched.filter(function(w){return w.type==='book'}).length,
    games_played:watched.filter(function(w){return w.type==='game'}).length,
    avg_rating:0,
    total:watched.length,
    watchlist:watchlist.length
  };
  var ratings=watched.filter(function(w){return w.rating>0});
  if(ratings.length>0)stats.avg_rating=Math.round(ratings.reduce(function(a,w){return a+w.rating},0)/ratings.length*10)/10;
  return stats;
}
