'use strict';
/* ENTERTAINMENT — ПОЛНЫЙ ДОСУГ: ФИЛЬМЫ, СЕРИАЛЫ, КНИГИ, МУЗЫКА, ИГРЫ, ПОДКАСТЫ, ТЕАТР, ИСКУССТВО */

/* ============ БИБЛИОТЕКА ФИЛЬМОВ ============ */
var MOVIES_LIBRARY=[
{id:'mov_01',title:'Побег из Шоушенка',year:1994,genre:'Драма',rating:9.3,director:'Фрэнк Дарабонт',desc:'История надежды и дружбы в тюрьме. Лучший фильм всех времён по IMDb.',why:'Учит терпению, надежде, силе духа.',lesson:'Надежда — это хорошо, возможно, самое лучшее.'},
{id:'mov_02',title:'Крёстный отец',year:1972,genre:'Криминал',rating:9.2,director:'Фрэнсис Форд Коппола',desc:'Сага о семье Корлеоне. Мафия, власть, семья.',why:'Учит стратегии, семейным ценностям, последствиям.',lesson:'Никогда не говори людям, что думаешь.'},
{id:'mov_03',title:'Тёмный рыцарь',year:2008,genre:'Боевик',rating:9.0,director:'Кристофер Нолан',desc:'Бэтмен против Джокера. Хаос против порядка.',why:'Учит морали, выбору, границам.',lesson:'Ты либо умираешь героем, либо живёшь до того, как станешь злодеем.'},
...
{id:'mov_06',title:'Начало',year:2010,genre:'Фантастика',rating:8.8,director:'Кристофер Нолан',desc:'Кража идей из снов. Что реально?',why:'Учит критическому мышлению, работе с подсознанием.',lesson:'Идеи — самый стойкий паразит.'},
...
{id:'mov_08',title:'Бойцовский клуб',year:1999,genre:'Драма',rating:8.8,director:'Дэвид Финчер',desc:'Потеря поколения. Бунт против системы.',why:'Учит осознанности, борьбе с потребительством.',lesson:'Ты — не твоя работа. Ты — не твои деньги.'},
...
{id:'mov_11',title:'Зелёная книга',year:2018,genre:'Драма',rating:8.2,director:'Питер Фаррелли',desc:'Расизм и дружба в 1960-х.',why:'Учит эмпатии, преодолению предрассудков.',lesson:'Достоинство выше обстоятельств.'},
...
{id:'bk_06',title:'Sapiens. Краткая история человечества',author:'Юваль Ной Харари',year:2011,genre:'История',rating:8.9,desc:'От обезьян до богов.',why:'Учит истории, антропологии, будущему.',lesson:'Мы — единственные животные, верящие в вымысел.'},
...
function getRandomMovie(){
  return MOVIES_LIBRARY[Math.floor(Math.random()*MOVIES_LIBRARY.length)];
}

function getRandomSeries(){
  return SERIES_LIBRARY[Math.floor(Math.random()*SERIES_LIBRARY.length)];
}

function getRandomBook(){
  return BOOKS_LIBRARY[Math.floor(Math.random()*BOOKS_LIBRARY.length)];
}

function getRandomGame(){
  return GAMES_LIBRARY[Math.floor(Math.random()*GAMES_LIBRARY.length)];
}

function getRandomPodcast(){
  return PODCASTS_LIBRARY[Math.floor(Math.random()*PODCASTS_LIBRARY.length)];
}

function searchMovies(query){
  var q=query.toLowerCase();
  return MOVIES_LIBRARY.filter(function(m){
    return m.title.toLowerCase().indexOf(q)>=0||
           m.genre.toLowerCase().indexOf(q)>=0||
           m.director.toLowerCase().indexOf(q)>=0;
  });
}

function searchBooks(query){
  var q=query.toLowerCase();
  return BOOKS_LIBRARY.filter(function(b){
...
function filterByGenre(list,genre){
  return list.filter(function(i){return i.genre===genre});
}

function getTopRated(list,minRating){
  return list.filter(function(i){return i.rating>=(minRating||8.5)})
...
function addToWatchlist(item,type){
  ...
  var exists=state.watchlist.some(function(w){return w.id===item.id});
  if(exists){
    toast('Уже в списке','warning');
    return false;
  }
...
function markAsWatched(itemId,rating,notes){
  if(!state.watched)state.watched=[];
  var item=(state.watchlist||[]).find(function(w){return w.id===itemId});
  ...
  state.watchlist=state.watchlist.filter(function(w){return w.id!==itemId});
  save();
  toast('Отмечено как просмотрено','success');
  return true;
}
...
function getRecommendations(mood){
  var recs={};
  if(mood==='focus'){
    recs.music=MUSIC_LIBRARY.filter(function(m){return m.mood==='focus'});
    recs.podcasts=PODCASTS_LIBRARY.slice(0,3);
  }else if(mood==='relax'){
    recs.movies=MOVIES_LIBRARY.filter(function(m){return m.genre==='Комедия'||m.genre==='Драма'}).slice(0,3);
    recs.music=MUSIC_LIBRARY.filter(function(m){return m.mood==='meditation'});
  }else if(mood==='learn'){
    recs.books=BOOKS_LIBRARY.filter(function(b){return b.genre==='Саморазвитие'||b.genre==='Психология'}).slice(0,3);
    recs.podcasts=PODCASTS_LIBRARY.filter(function(p){return p.genre==='Нейробиология'||p.genre==='Наука/Философия'});
  }else if(mood==='inspire'){
    recs.movies=MOVIES_LIBRARY.filter(function(m){return m.rating>=8.8}).slice(0,3);
    recs.books=BOOKS_LIBRARY.filter(function(b){return b.rating>=8.8}).slice(0,3);
  }
  return recs;
}

/* ============ СТАТИСТИКА ДОСУГА ============ */
function getEntertainmentStats(){
  var stats={
    movies_watched:(state.watched||[]).filter(function(w){return w.type==='movie'}).length,
    series_watched:(state.watched||[]).filter(function(w){return w.type==='series'}).length,
    books_read:(state.watched||[]).filter(function(w){return w.type==='book'}).length,
    games_played:(state.watched||[]).filter(function(w){return w.type==='game'}).length,
    avg_rating:0,
    total:(state.watched||[]).length,
    watchlist:(state.watchlist||[]).length
  };
  var ratings=(state.watched||[]).filter(function(w){return w.rating>0});
  if(ratings.length>0){
    stats.avg_rating=Math.round(ratings.reduce(function(a,w){return a+w.rating},0)/ratings.length*10)/10;
  }
  return stats;
}
