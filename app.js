...
/* ============================================================
   ...
   ╔══════════════════════════════════════════════════════════╗
   ║  🔒 ЗОЛОТЫЕ ПРАВИЛА СОХРАНЕНИЯ ДАННЫХ                   ║
   ║                                                          ║
   ║  1. Ключ 'life_os_v40' НИКОГДА НЕ МЕНЯЕТСЯ              ║
   ║     Меняется только версия ВНУТРИ (settings.version)    ║
   ║                                                          ║
   ║  2. При загрузке: миграция через deepMerge              ║
   ║     Новые поля ДОБАВЛЯЮТСЯ, старые СОХРАНЯЮТСЯ          ║
   ║                                                          ║
   ║  3. Перед загрузкой — бэкап в 'life_os_backup'          ║
   ║                                                          ║
   ║  4. НИКОГДА не localStorage.clear() без confirm()       ║
   ║                                                          ║
   ║  5. Экспорт JSON доступен всегда                        ║
   ║                                                          ║
   ║  6. save() вызывается после каждого изменения           ║
   ║     + автосейв каждые 30 сек                            ║
   ╚══════════════════════════════════════════════════════════╝
   ============================================================ */

/* ============ КОНСТАНТЫ ХРАНИЛИЩА ============ */
var STORAGE_KEY = 'life_os_v40';           // ⚠️ НЕ МЕНЯТЬ НИКОГДА
var STORAGE_BACKUP = 'life_os_backup';     // резервная копия
...
/* ============================================================
   DEFAULT STATE — все поля, которые могут быть в приложении
   ============================================================ */
function defaultState(){
  return{
    ...
    detoxCourseProgress:{},
    challengeProgress:{},dailySurveys:{},
    dayPlans:{},weekPlans:{},monthPlans:{},

    /* Прогресс курсов (добавлено в v41) */
...
/* ============================================================
   🔒 БЭКАП — копия перед любой операцией
   ============================================================ */
function backupStorage(){
  try{
    var raw=localStorage.getItem(STORAGE_KEY);
    if(raw){
      localStorage.setItem(STORAGE_BACKUP,raw);
      console.log('[BACKUP] ✓ Копия сохранена в',STORAGE_BACKUP);
      return true;
    }
...
function restoreFromBackup(){
  try{
    ...
    console.log('[RESTORE] ✓ Восстановлено из бэкапа');
    return true;
  }catch(e){return false}
}

/* ============================================================
   🔒 DEEP MERGE — новые поля добавляются, старые СОХРАНЯЮТСЯ
   Правило: source (данные пользователя) ПОБЕЖДАЕТ
   ============================================================ */
...
/* ============================================================
   🔒 МИГРАЦИЯ — обновление версии БЕЗ потери данных
   ============================================================ */
function migrateState(data){
  ...
  if(v<parseFloat(CURRENT_VERSION)){
    ...
    // Добавляем недостающие поля из defaultState
...
/* ============================================================
   🔒 ЗАГРУЗКА STATE — с бэкапом и миграцией
   ============================================================ */
var state;
(function loadState(){
  try{
    ...
    if(raw){
      ...
      console.log('[LOAD] ✓ state загружен (v'+state.settings.version+')');
    }else{
      state=defaultState();
      console.log('[LOAD] новый state создан');
    }
  }catch(e){
    console.error('[LOAD] ошибка:',e);
    if(!restoreFromBackup()){
      state=defaultState();
    }else{
      try{
        var raw2=localStorage.getItem(STORAGE_KEY);
        state=deepMerge(defaultState(),JSON.parse(raw2));
      }catch(e2){state=defaultState()}
    }
  }
})();

/* ============================================================
   🔒 СОХРАНЕНИЕ — вызывается после каждого изменения
   ============================================================ */
function save(){
  try{
...
function resetAllWithConfirm(){
  if(!confirm('⚠️ Удалить ВСЕ данные? Бэкап сохранён в life_os_backup.'))return;
...
/* ============================================================
   ТЕМЫ — ЭФФЕКТЫ (сильные и дорогие)
   ============================================================ */
function startEffects(){
  ...
  try{
    ...
    function cnt(base){return Math.round(base*intensity*2.5)}  // ← усилено в 2.5×
    var i,s,d,el;

    if(effect==='stars'){
      // 3 слоя звёзд — мелкие, средние, крупные
      ...
      for(i=0;i<cnt(50);i++){el=document.createElement('div');el.className='effect-drop';el.style.left=Math.random()*100+'%';el.style.animationDuration=(0.8+Math.random()*1.2)+'s';el.style.animationDelay=Math.random()*4+'s';el.style.height=(15+Math.random()*35)+'px';el.style.opacity=(0.3+Math.random()*0.5);overlay.appendChild(el)}
    }
      ...
      for(i=0;i<5;i++){el=document.createElement('div');el.className='effect-wave';el.style.bottom=(i*40)+'px';el.style.animationDelay=(i*1.2)+'s';el.style.opacity=(0.15+Math.random()*0.1);overlay.appendChild(el)}
    }
      ...
      for(i=0;i<cnt(3);i++){el=document.createElement('div');el.className='effect-lightning';el.style.left=(20+Math.random()*60)+'%';el.style.animationDelay=(Math.random()*6)+'s';overlay.appendChild(el)}
    }
    ...
    // Fallback — если ничего не создано, звёзды
...
function applyTheme(id){
  try{
    // Снимаем все theme-* классы, оставляем theme-dark как fallback
...
/* ============================================================
   ...
   Все разделы рабочие, без "в разработке"
   ============================================================ */
...
function renderMethods(){
  ...
  (window.METHODS_LIBRARY||[]).forEach(function(m){
    var done=state.methodsProgress&&state.methodsProgress[m.id];
    html+='<div class="method-card" onclick="openMethod(\''+m.id+'\')"><div class="method-header"><div class="method-emoji">'+m.emoji+'</div><div style="flex:1;"><div class="method-title">'+m.title+'</div><div class="method-cat">'+m.category+'</div></div>'+(done?'<span class="badge badge-success">✓</span>':'<div class="list-chevron">›</div>')+'</div><div class="method-desc">'+m.desc+'</div></div>';
  });
  html+='</div>';
  document.getElementById('app').innerHTML=html;
}
function openMethod(id){
  ...
  var done=state.methodsProgress&&state.methodsProgress[id];
  ...
  if(!done)html+='<button class="btn btn-primary btn-block mt-3" onclick="completeMethod(\''+m.id+'\')">✓ Изучено</button>';
  else html+='<div class="badge badge-success" style="display:block;text-align:center;padding:12px;">✓ Изучено</div>';
  openSheet(m.title,html);
}
function completeMethod(id){
  if(!state.methodsProgress)state.methodsProgress={};
  state.methodsProgress[id]=true;
  state.xp=(state.xp||0)+15;
  save();haptic('success');toast('✓ Изучено! +15 XP','success');
  checkAchievements();closeSheet();renderMethods();
}
window.openMethod=openMethod;
window.completeMethod=completeMethod;
...
function completePsychology(id){
  ...
  state.psychologyProgress[id]=true;
  state.xp=(state.xp||0)+15;
  save();haptic('success');toast('✓ Изучено! +15 XP','success');
  checkAchievements();closeSheet();renderPsychology();
}
...
function completeThinking(id){
  ...
  state.thinkingProgress[id]=true;
  state.xp=(state.xp||0)+15;
  save();haptic('success');toast('✓ Изучено! +15 XP','success');
  checkAchievements();closeSheet();renderThinking();
}
...
function completeEtiquette(id){
  ...
  state.etiquetteProgress[id]=true;
  state.xp=(state.xp||0)+10;
  save();haptic('success');toast('✓ Изучено! +10 XP','success');
  checkAchievements();closeSheet();renderEtiquette();
}
...
function completeHormone(id){
  ...
  state.hormonesProgress[id]=true;
  state.xp=(state.xp||0)+20;
  save();haptic('success');toast('✓ Изучено! +20 XP','success');
  checkAchievements();closeSheet();renderHormones();
}
...
function completeWealth(id){
  ...
  state.wealthProgress[id]=true;
  state.xp=(state.xp||0)+15;
  save();haptic('success');toast('✓ Изучено! +15 XP','success');
  checkAchievements();closeSheet();renderWealth();
}
...
/* ============================================================
   MEMORY — рабочий раздел
   ============================================================ */
var MEMORY_TECHNIQUES=[
{id:'mem_palace',emoji:'🏛',title:'Дворец памяти',desc:'Метод локусов',theory:'**Привяжи образы к знакомому месту.** 10 локусов = 10 объектов. Чем страннее образ — тем крепче память.',science:'Метод Цицерона, чемпионы мира по памяти.',practice:['Выбери дом','10 точек по маршруту','Размести образы','Пройди маршрут 3×'],effect:'×5 ёмкость',tips:'Странные + эмоциональные образы'},
{id:'mem_chunking',emoji:'🧩',title:'Chunking',desc:'Разбивка на блоки',theory:'**7±2 — предел рабочей памяти.** Разбивай 12 цифр на 4 блока по 3.',science:'Миллер 1956.',practice:['Возьми телефон','Разбей на 3-4 блока','Запомни','Проверь'],effect:'+300%',tips:'Осмысленные блоки'},
{id:'mem_spacing',emoji:'📅',title:'Интервальное повторение',desc:'Spaced repetition',theory:'**Забываем 58% за 20 мин.** Повторяй 1д / 3д / 7д / 14д / 30д.',science:'Эббингауз.',practice:['Карточки Anki','1д/3д/7д/14д/30д','20 мин/день'],effect:'×3 удержание',tips:'Чем сложнее — тем чаще'},
{id:'mem_association',emoji:'🔗',title:'Ассоциации',desc:'Связывание',theory:'**Новое через знакомое.** Запоминай через рифмы, истории, образы.',science:'Мнемоника.',practice:['Возьми 10 слов','Придумай историю','Свяжи с известным','Повтори'],effect:'+200%',tips:'Эмоции усиливают'},
{id:'mem_mnemonic',emoji:'🎵',title:'Мнемоники',desc:'Рифмы и аббревиатуры',theory:'**"Каждый Охотник Желает Знать"** — цвета радуги. 20+ мнемоник для всего.',science:'Ассоциативное запоминание.',practice:['Придумай 5','Рифмуй','Сокращай'],effect:'+150%',tips:'Личные работают лучше'},
{id:'mem_active',emoji:'🔄',title:'Recall > Recognition',desc:'Активное вспоминание',theory:'**Вспомни сам — в 3 раза эффективнее, чем перечитать.**',science:'Roediger & Karpicke 2006.',practice:['Закрой книгу','Расскажи вслух','Проверь','Повтори'],effect:'×3',tips:'Тестируй себя'},
{id:'mem_dual',emoji:'👁',title:'Двойное кодирование',desc:'Текст + образ',theory:'**Мозг любит картинки.** Свяжи слово с образом.',science:'Паивио.',practice:['Слово → картинка','Нарисуй','Запомни'],effect:'×2',tips:'Свои образы'},
{id:'mem_sleep',emoji:'😴',title:'Сон + память',desc:'Консолидация',theory:'**Глубокий сон консолидирует факты, REM — эмоции.** Учи перед сном.',science:'Уокер.',practice:['Повтори за 30 мин до сна','7-9 ч сна','Утром проверь'],effect:'+40%',tips:'Сон критичен'}
];

function renderMemory(){
  var done=Object.keys(state.memoryProgress||{}).length;
  var html='<div class="page"><div class="title-xl">🧠 Память</div>';
  html+='<div class="card card-gradient"><div style="opacity:.9;font-size:12px;">Прогресс</div><div style="font-size:40px;font-weight:800;">'+done+'/'+MEMORY_TECHNIQUES.length+'</div></div>';
...
function openMemoryTechnique(id){
  ...
  var html='<div style="text-align:center;margin-bottom:16px;"><div style="font-size:48px;">'+m.emoji+'</div><div style="font-size:20px;font-weight:800;">'+m.title+'</div><div class="footnote text-secondary">'+m.desc+'</div></div>';
  html+='<div class="card"><div class="lesson-section"><div class="lesson-section-title">📚 Теория</div><div class="lesson-content">'+formatLesson(m.theory)+'</div></div></div>';
  html+='<div class="card"><div class="lesson-section science"><div class="lesson-section-title">🔬 Наука</div><div class="lesson-content">'+formatLesson(m.science)+'</div></div></div>';
  ...
  html+='<div class="insight-card"><div class="insight-title">💎 Эффект</div><div class="insight-text">'+esc(m.effect)+'</div></div>';
  html+='<div class="insight-card"><div class="insight-title">💡 Совет</div><div class="insight-text">'+esc(m.tips)+'</div></div>';
...
function completeMemory(id){
  ...
  state.memoryProgress[id]=true;
  state.xp=(state.xp||0)+15;
  save();haptic('success');toast('✓ Изучено! +15 XP','success');
  checkAchievements();closeSheet();renderMemory();
}
...
/* ============================================================
   ...
   ============================================================ */
var IQ_TESTS=[
{id:'iq_raven',emoji:'🔷',title:'Матрицы Равена',desc:'Логические паттерны',theory:'**Классический тест IQ.** Найди закономерность в матрице.',science:'Равен 1936.',practice:['Пройти тест','Разобрать ошибки','Тренировать 10 мин/день'],effect:'+5-15 IQ',tips:'Регулярно'},
{id:'iq_nback',emoji:'🎯',title:'N-back',desc:'Рабочая память',theory:'**Запоминай N символов назад.** Тренирует fluid IQ.',science:'Jaeggi 2008: +20% за 4 недели.',practice:['N=2 → N=4','15 мин/день','30 дней'],effect:'+20% fluid IQ',tips:'Киршнер'},
{id:'iq_analogies',emoji:'⚖️',title:'Аналогии',desc:'Вербальный IQ',theory:'**A:B = C:?**. Тренируй связи между понятиями.',science:'Вербальный интеллект.',practice:['10 аналогий/день','Записывай связь'],effect:'+вербальный',tips:'Скорость'},
{id:'iq_math',emoji:'🔢',title:'Устный счёт',desc:'Скорость',theory:'**Тренируй мозг считать в уме.** 5 примеров в день.',science:'Рабочая память.',practice:['5 примеров/день','Ускоряйся','Только в уме'],effect:'+скорость',tips:'Ежедневно'},
{id:'iq_logic',emoji:'🧩',title:'Логические задачи',desc:'Дедукция',theory:'**Судоку, шахматы, нонограммы.**',science:'Тренировка логики.',practice:['Судоку 1/день','Шахматы 3×/нед'],effect:'+логика',tips:'Сложность ↑'},
{id:'iq_dual',emoji:'🎲',title:'Dual N-back',desc:'Двойная задача',theory:'**Запоминай позицию + звук.**',science:'Jaeggi 2008.',practice:['15 мин/день','4 недели'],effect:'+20% fluid IQ',tips:'Обе задачи одновременно'}
];

function renderIQ(){
  var done=Object.keys(state.iqProgress||{}).length;
  var html='<div class="page"><div class="title-xl">🎯 IQ</div>';
  html+='<div class="card card-gradient"><div style="opacity:.9;font-size:12px;">Прогресс</div><div style="font-size:40px;font-weight:800;">'+done+'/'+IQ_TESTS.length+'</div></div>';
  IQ_TESTS.forEach(function(t){
    var isDone=state.iqProgress&&state.iqProgress[t.id];
    html+='<div class="method-card" onclick="openIQTest(\''+t.id+'\')"><div class="method-header"><div class="method-emoji">'+t.emoji+'</div><div style="flex:1;"><div class="method-title">'+t.title+'</div><div class="method-cat">'+t.desc+'</div></div>'+(isDone?'<span class="badge badge-success">✓</span>':'<div class="list-chevron">›</div>')+'</div></div>';
  });
  html+='</div>';
  document.getElementById('app').innerHTML=html;
}
function openIQTest(id){
  var t=IQ_TESTS.find(function(x){return x.id===id});if(!t)return;
  var isDone=state.iqProgress&&state.iqProgress[id];
  ...
  html+='<div class="card"><div class="lesson-section"><div class="lesson-section-title">📚 Теория</div><div class="lesson-content">'+formatLesson(t.theory)+'</div></div></div>';
  html+='<div class="card"><div class="lesson-section science"><div class="lesson-section-title">🔬 Наука</div><div class="lesson-content">'+formatLesson(t.science)+'</div></div></div>';
  html+='<div class="card"><div class="lesson-section practice"><div class="lesson-section-title">🎯 Практика</div><div class="lesson-content"><ul>'+t.practice.map(function(p){return '<li>'+esc(p)+'</li>'}).join('')+'</ul></div></div></div>';
  html+='<div class="insight-card"><div class="insight-title">💎 Эффект</div><div class="insight-text">'+esc(t.effect)+'</div></div>';
  if(!isDone)html+='<button class="btn btn-primary btn-block mt-3" onclick="completeIQ(\''+t.id+'\')">✓ Изучено</button>';
  else html+='<div class="badge badge-success" style="display:block;text-align:center;padding:12px;">✓ Изучено</div>';
  openSheet(t.title,html);
}
function completeIQ(id){
  if(!state.iqProgress)state.iqProgress={};
  state.iqProgress[id]=true;
  state.xp=(state.xp||0)+15;
  save();haptic('success');toast('✓ Изучено! +15 XP','success');
  checkAchievements();closeSheet();renderIQ();
}
window.openIQTest=openIQTest;
window.completeIQ=completeIQ;
...
/* ============================================================
   ...
   ============================================================ */
var EQ_MODULES=[
{id:'eq_self',emoji:'🪞',title:'Самосознание',desc:'Знай себя',theory:'**Замечай эмоции без осуждения.** Назови — ослабь.',science:'Гоулман.',practice:['Дневник эмоций 3×/день','"Я чувствую..."'],effect:'+50% EQ',tips:'Без "плохо/хорошо"'},
{id:'eq_regulate',emoji:'🎯',title:'Саморегуляция',desc:'Управляй реакцией',theory:'**Пауза 6 сек между стимулом и реакцией.**',science:'Виктор Франкл.',practice:['Дыхание 4-7-8','Пауза 6 сек','Рефрейминг'],effect:'-импульсивность',tips:'Пауза = сила'},
{id:'eq_motivation',emoji:'🚀',title:'Мотивация',desc:'Смысл > награды',theory:'**Автономия, компетентность, связанность (Деси).**',science:'Деси & Райан.',practice:['Найди "зачем"','Микроцели','Празднуй'],effect:'+действие',tips:'Внутреннее'},
{id:'eq_empathy',emoji:'❤️',title:'Эмпатия',desc:'Понимай других',theory:'**3 типа: когнитивная, эмоциональная, сострадательная.**',science:'Гоулман.',practice:['Активное слушание','Парафраз','"На его месте"'],effect:'+связи',tips:'Слушай 70%'},
{id:'eq_social',emoji:'👥',title:'Социальные навыки',desc:'Влияние',theory:'**Конфликты → решения. Связи → ресурсы.**',science:'Гоулман.',practice:['ННО','Я-сообщения','SBI-фидбэк'],effect:'+успех',tips:'Win-win'},
{id:'eq_anger',emoji:'😤',title:'Управление гневом',desc:'Пауза',theory:'**Пик гнева 90 сек.**',science:'Кора + миндалина.',practice:['Пауза','Спорт','Дыхание'],effect:'-конфликты',tips:'Сигнал'},
{id:'eq_anxiety',emoji:'😰',title:'Тревога',desc:'Спокойствие',theory:'**Тревога = предсказание без фактов.**',science:'КПТ.',practice:['4-7-8','Дневник','КПТ'],effect:'-тревога',tips:'Что реально?'},
{id:'eq_joy',emoji:'😊',title:'Тренировка радости',desc:'Позитив',theory:'**Замечай 3 хороших в день.**',science:'Селигман.',practice:['3 хороших','Благодарность','Смех'],effect:'+25%',tips:'Тренируется'}
];

function renderEQ(){
  var done=Object.keys(state.eqProgress||{}).length;
  var html='<div class="page"><div class="title-xl">❤️ EQ</div>';
  html+='<div class="card card-gradient"><div style="opacity:.9;font-size:12px;">Прогресс</div><div style="font-size:40px;font-weight:800;">'+done+'/'+EQ_MODULES.length+'</div></div>';
  EQ_MODULES.forEach(function(m){
    var isDone=state.eqProgress&&state.eqProgress[m.id];
    html+='<div class="method-card" onclick="openEQModule(\''+m.id+'\')"><div class="method-header"><div class="method-emoji">'+m.emoji+'</div><div style="flex:1;"><div class="method-title">'+m.title+'</div><div class="method-cat">'+m.desc+'</div></div>'+(isDone?'<span class="badge badge-success">✓</span>':'<div class="list-chevron">›</div>')+'</div></div>';
  });
  html+='</div>';
  document.getElementById('app').innerHTML=html;
}
function openEQModule(id){
  var m=EQ_MODULES.find(function(x){return x.id===id});if(!m)return;
  var isDone=state.eqProgress&&state.eqProgress[id];
  var html='<div style="text-align:center;margin-bottom:16px;"><div style="font-size:48px;">'+m.emoji+'</div><div style="font-size:20px;font-weight:800;">'+m.title+'</div></div>';
  html+='<div class="card"><div class="lesson-section"><div class="lesson-section-title">📚 Теория</div><div class="lesson-content">'+formatLesson(m.theory)+'</div></div></div>';
  html+='<div class="card"><div class="lesson-section science"><div class="lesson-section-title">🔬 Наука</div><div class="lesson-content">'+formatLesson(m.science)+'</div></div></div>';
  html+='<div class="card"><div class="lesson-section practice"><div class="lesson-section-title">🎯 Практика</div><div class="lesson-content"><ul>'+m.practice.map(function(p){return '<li>'+esc(p)+'</li>'}).join('')+'</ul></div></div></div>';
  html+='<div class="insight-card"><div class="insight-title">💎 Эффект</div><div class="insight-text">'+esc(m.effect)+'</div></div>';
  if(!isDone)html+='<button class="btn btn-primary btn-block mt-3" onclick="completeEQ(\''+m.id+'\')">✓ Изучено</button>';
  else html+='<div class="badge badge-success" style="display:block;text-align:center;padding:12px;">✓ Изучено</div>';
  openSheet(m.title,html);
}
function completeEQ(id){
  if(!state.eqProgress)state.eqProgress={};
  state.eqProgress[id]=true;
  state.xp=(state.xp||0)+15;
  save();haptic('success');toast('✓ Изучено! +15 XP','success');
  checkAchievements();closeSheet();renderEQ();
}
window.openEQModule=openEQModule;
window.completeEQ=completeEQ;
window.renderEQ=renderEQ;

/* ============================================================
   FINANCE — рабочий раздел
   ============================================================ */
var FINANCE_MODULES=[
{id:'fin_budget',emoji:'💰',title:'Бюджет 50/30/20',desc:'Основа',theory:'**50% нужды, 30% желания, 20% сбережения.**',science:'Элизабет Уоррен.',practice:['Записывай 30 дней','Разбей доход','Отдельный счёт'],effect:'+контроль',tips:'Автоматизация'},
{id:'fin_save',emoji:'🏦',title:'Подушка 3-6 мес',desc:'Безопасность',theory:'**3-6 месяцев расходов на чёрный день.**',science:'Финансовая устойчивость.',practice:['Открой счёт','Автоперевод','Не трогай'],effect:'+спокойствие',tips:'Не инвестируй'},
{id:'fin_invest',emoji:'📈',title:'Инвестиции',desc:'Деньги работают',theory:'**Индексные фонды + DCA + долгосрок.** S&P 500 = +10%/год.',science:'Сложный процент.',practice:['3 фонда','DCA ежемесячно','Ревью/квартал'],effect:'+капитал',tips:'Время > тайминг'},
{id:'fin_debt',emoji:'⛓',title:'Освобождение от долгов',desc:'Свобода',theory:'**Снежный ком (мелкий → крупный) или лавина (высокий %).**',science:'Dave Ramsey.',practice:['Список долгов','Минимум по всем','Максимум на 1'],effect:'-долги',tips:'Ком → мотивация'},
{id:'fin_income',emoji:'💼',title:'Доход 3+ источника',desc:'Стабильность',theory:'**Основной + фриланс + пассивный.**',science:'Диверсификация.',practice:['Навык','Первые 1000 ₽','Система'],effect:'+стабильность',tips:'Диверсификация'},
{id:'fin_fire',emoji:'🔥',title:'FIRE',desc:'Независимость',theory:'**25× годовых + правило 4%.**',science:'Trinity Study.',practice:['Норма сбережений 50%','Диверсификация','25× цель'],effect:'Свобода',tips:'Сокращай'},
{id:'fin_taxes',emoji:'📋',title:'Налоги и вычеты',desc:'Возврат',theory:'**ИИС, вычеты, самозанятость → до 15% назад.**',science:'НК РФ.',practice:['ИИС','Вычеты','Документы'],effect:'+возврат',tips:'Ежегодно'},
{id:'fin_career',emoji:'📊',title:'Карьера',desc:'Рост дохода',theory:'**T-shape: широкий + глубокий в 1.**',science:'Harvard Business Review.',practice:['Навык','Ментор','Переговоры'],effect:'+20%',tips:'Постоянно'}
];

function renderFinance(){
  var done=Object.keys(state.financeProgress||{}).length;
  var html='<div class="page"><div class="title-xl">💰 Финансы</div>';
  html+='<div class="card card-gradient"><div style="opacity:.9;font-size:12px;">Прогресс</div><div style="font-size:40px;font-weight:800;">'+done+'/'+FINANCE_MODULES.length+'</div></div>';
  FINANCE_MODULES.forEach(function(m){
    var isDone=state.financeProgress&&state.financeProgress[m.id];
    html+='<div class="method-card" onclick="openFinanceModule(\''+m.id+'\')"><div class="method-header"><div class="method-emoji">'+m.emoji+'</div><div style="flex:1;"><div class="method-title">'+m.title+'</div><div class="method-cat">'+m.desc+'</div></div>'+(isDone?'<span class="badge badge-success">✓</span>':'<div class="list-chevron">›</div>')+'</div></div>';
  });
  html+='</div>';
  document.getElementById('app').innerHTML=html;
}
function openFinanceModule(id){
  var m=FINANCE_MODULES.find(function(x){return x.id===id});if(!m)return;
  var isDone=state.financeProgress&&state.financeProgress[id];
  var html='<div style="text-align:center;margin-bottom:16px;"><div style="font-size:48px;">'+m.emoji+'</div><div style="font-size:20px;font-weight:800;">'+m.title+'</div></div>';
  html+='<div class="card"><div class="lesson-section"><div class="lesson-section-title">📚 Теория</div><div class="lesson-content">'+formatLesson(m.theory)+'</div></div></div>';
  html+='<div class="card"><div class="lesson-section science"><div class="lesson-section-title">🔬 Наука</div><div class="lesson-content">'+formatLesson(m.science)+'</div></div></div>';
  html+='<div class="card"><div class="lesson-section practice"><div class="lesson-section-title">🎯 Практика</div><div class="lesson-content"><ul>'+m.practice.map(function(p){return '<li>'+esc(p)+'</li>'}).join('')+'</ul></div></div></div>';
  html+='<div class="insight-card"><div class="insight-title">💎 Эффект</div><div class="insight-text">'+esc(m.effect)+'</div></div>';
  if(!isDone)html+='<button class="btn btn-primary btn-block mt-3" onclick="completeFinance(\''+m.id+'\')">✓ Изучено</button>';
  else html+='<div class="badge badge-success" style="display:block;text-align:center;padding:12px;">✓ Изучено</div>';
  openSheet(m.title,html);
}
function completeFinance(id){
  if(!state.financeProgress)state.financeProgress={};
  state.financeProgress[id]=true;
  state.xp=(state.xp||0)+15;
  save();haptic('success');toast('✓ Изучено! +15 XP','success');
  checkAchievements();closeSheet();renderFinance();
}
window.openFinanceModule=openFinanceModule;
window.completeFinance=completeFinance;
window.renderFinance=renderFinance;

/* ============================================================
   NEURO — рабочий раздел
   ============================================================ */
var NEURO_TOPICS=[
{id:'neu_plasticity',emoji:'🧬',title:'Нейропластичность',desc:'Мозг меняется',theory:'**Мозг меняется всю жизнь.** Новое → стресс → рост. 30 дней → новые связи.',science:'Дуа, Мерцених.',practice:['30 дней 1 навык','Сон','Повторение'],effect:'+рост',tips:'Постоянно'},
{id:'neu_dopamine',emoji:'⚡',title:'Дофамин',desc:'Мотивация',theory:'**Предвкушение, не удовольствие.** Дешёвые стимулы → яма.',science:'Шульц.',practice:['Дофамин-детокс 1×/нед','Утро без телефона'],effect:'+мотивация',tips:'См. "Гормоны"'},
{id:'neu_sleep',emoji:'😴',title:'Сон и память',desc:'Консолидация',theory:'**Глубокий сон = факты, REM = эмоции.**',science:'Уокер.',practice:['7-9 ч','Учи перед сном','Утром проверь'],effect:'+40%',tips:'Сон критичен'},
{id:'neu_bdnf',emoji:'🏃',title:'BDNF',desc:'Рост нейронов',theory:'**Спорт + голодание + обучение → BDNF ↑.**',science:'Ratey.',practice:['30 мин кардио','16:8','Новое'],effect:'+память',tips:'Учи после спорта'},
{id:'neu_myelin',emoji:'⚡',title:'Миелинизация',desc:'Быстрые связи',theory:'**Повторение → миелин → скорость ×10.**',science:'Fields.',practice:['Повторяй','Интервалы','Медленно'],effect:'+скорость',tips:'Регулярно'},
{id:'neu_habits',emoji:'🔄',title:'Нейробиология привычек',desc:'Автоматизм',theory:'**Cue→Craving→Response→Reward. 66 дней.**',science:'Дахигг.',practice:['1 привычка','Привязка','Награда'],effect:'+автоматизм',tips:'Медленно'}
];

function renderNeuro(){
  var done=Object.keys(state.neuroProgress||{}).length;
  var html='<div class="page"><div class="title-xl">🔬 Нейро</div>';
  html+='<div class="card card-gradient"><div style="opacity:.9;font-size:12px;">Прогресс</div><div style="font-size:40px;font-weight:800;">'+done+'/'+NEURO_TOPICS.length+'</div></div>';
  NEURO_TOPICS.forEach(function(t){
    var isDone=state.neuroProgress&&state.neuroProgress[t.id];
    html+='<div class="method-card" onclick="openNeuroTopic(\''+t.id+'\')"><div class="method-header"><div class="method-emoji">'+t.emoji+'</div><div style="flex:1;"><div class="method-title">'+t.title+'</div><div class="method-cat">'+t.desc+'</div></div>'+(isDone?'<span class="badge badge-success">✓</span>':'<div class="list-chevron">›</div>')+'</div></div>';
  });
  html+='</div>';
  document.getElementById('app').innerHTML=html;
}
function openNeuroTopic(id){
  var t=NEURO_TOPICS.find(function(x){return x.id===id});if(!t)return;
  var isDone=state.neuroProgress&&state.neuroProgress[id];
  var html='<div style="text-align:center;margin-bottom:16px;"><div style="font-size:48px;">'+t.emoji+'</div><div style="font-size:20px;font-weight:800;">'+t.title+'</div></div>';
  html+='<div class="card"><div class="lesson-section"><div class="lesson-section-title">📚 Теория</div><div class="lesson-content">'+formatLesson(t.theory)+'</div></div></div>';
  html+='<div class="card"><div class="lesson-section science"><div class="lesson-section-title">🔬 Наука</div><div class="lesson-content">'+formatLesson(t.science)+'</div></div></div>';
  html+='<div class="card"><div class="lesson-section practice"><div class="lesson-section-title">🎯 Практика</div><div class="lesson-content"><ul>'+t.practice.map(function(p){return '<li>'+esc(p)+'</li>'}).join('')+'</ul></div></div></div>';
  html+='<div class="insight-card"><div class="insight-title">💎 Эффект</div><div class="insight-text">'+esc(t.effect)+'</div></div>';
  if(!isDone)html+='<button class="btn btn-primary btn-block mt-3" onclick="completeNeuro(\''+t.id+'\')">✓ Изучено</button>';
  else html+='<div class="badge badge-success" style="display:block;text-align:center;padding:12px;">✓ Изучено</div>';
  openSheet(t.title,html);
}
function completeNeuro(id){
  if(!state.neuroProgress)state.neuroProgress={};
  state.neuroProgress[id]=true;
  state.xp=(state.xp||0)+15;
  save();haptic('success');toast('✓ Изучено! +15 XP','success');
  checkAchievements();closeSheet();renderNeuro();
}
window.openNeuroTopic=openNeuroTopic;
window.completeNeuro=completeNeuro;
window.renderNeuro=renderNeuro;
...
function renderRecovery(){
  ...
  Object.keys(cats).forEach(function(cat){
    ...
    cats[cat].forEach(function(r){
      ...
      html+='<div class="list-row" onclick="openRecoveryItem(\''+r.id+'\')"><div class="list-icon">'+r.emoji+'</div><div class="list-body"><div class="list-title">'+r.title+'</div><div class="list-subtitle">'+r.time+' · '+r.effect+'</div></div>'+(isDone?'<span class="badge badge-success">✓</span>':'<div class="list-chevron">›</div>')+'</div>';
    });
    html+='</div>';
  });
  html+='</div>';
...
function completeRecovery(id){
  ...
  state.recoveryProgress[id]=true;
  state.xp=(state.xp||0)+10;
  save();haptic('success');toast('✓ Изучено! +10 XP','success');
  checkAchievements();closeSheet();renderRecovery();
}
...
function renderMovies(){
  var list=state.watchlist.filter(function(w){return w.type==='movie'})||[];
  var html='<div class="page"><div class="row-between" style="margin-bottom:14px;"><div class="title-xl" style="margin:0;">🎥 Фильмы</div><button class="btn btn-primary btn-sm" onclick="addWatchItem(\'movie\')">+</button></div>';
  if(list.length){list.forEach(function(m){html+='<div class="list-row"><div class="list-icon">🎥</div><div class="list-body"><div class="list-title">'+esc(m.title)+'</div><div class="list-subtitle">'+(m.rating?'⭐ '+m.rating:'—')+'</div></div><button class="btn btn-ghost btn-xs" onclick="event.stopPropagation();removeWatchItem(\''+m.id+'\')">🗑</button></div>'})}
  else{html+='<div class="empty"><div class="empty-icon">🎥</div><div class="empty-title">Пусто</div><div class="empty-text">Добавь фильм</div></div>'}
  html+='</div>';
  document.getElementById('app').innerHTML=html;
}
function renderSeries(){
  var list=state.watchlist.filter(function(w){return w.type==='series'})||[];
  var html='<div class="page"><div class="row-between" style="margin-bottom:14px;"><div class="title-xl" style="margin:0;">📺 Сериалы</div><button class="btn btn-primary btn-sm" onclick="addWatchItem(\'series\')">+</button></div>';
  if(list.length){list.forEach(function(m){html+='<div class="list-row"><div class="list-icon">📺</div><div class="list-body"><div class="list-title">'+esc(m.title)+'</div><div class="list-subtitle">'+(m.rating?'⭐ '+m.rating:'—')+'</div></div><button class="btn btn-ghost btn-xs" onclick="event.stopPropagation();removeWatchItem(\''+m.id+'\')">🗑</button></div>'})}
  else{html+='<div class="empty"><div class="empty-icon">📺</div><div class="empty-title">Пусто</div><div class="empty-text">Добавь сериал</div></div>'}
  html+='</div>';
  document.getElementById('app').innerHTML=html;
}
function renderBooks(){
  var list=state.watchlist.filter(function(w){return w.type==='book'})||[];
  var html='<div class="page"><div class="row-between" style="margin-bottom:14px;"><div class="title-xl" style="margin:0;">📚 Книги</div><button class="btn btn-primary btn-sm" onclick="addWatchItem(\'book\')">+</button></div>';
  if(list.length){list.forEach(function(m){html+='<div class="list-row"><div class="list-icon">📚</div><div class="list-body"><div class="list-title">'+esc(m.title)+'</div><div class="list-subtitle">'+(m.rating?'⭐ '+m.rating:'—')+'</div></div><button class="btn btn-ghost btn-xs" onclick="event.stopPropagation();removeWatchItem(\''+m.id+'\')">🗑</button></div>'})}
  else{html+='<div class="empty"><div class="empty-icon">📚</div><div class="empty-title">Пусто</div><div class="empty-text">Добавь книгу</div></div>'}
  html+='</div>';
  document.getElementById('app').innerHTML=html;
}
function renderMusic(){
  var list=state.watchlist.filter(function(w){return w.type==='music'})||[];
  var html='<div class="page"><div class="row-between" style="margin-bottom:14px;"><div class="title-xl" style="margin:0;">🎵 Музыка</div><button class="btn btn-primary btn-sm" onclick="addWatchItem(\'music\')">+</button></div>';
  if(list.length){list.forEach(function(m){html+='<div class="list-row"><div class="list-icon">🎵</div><div class="list-body"><div class="list-title">'+esc(m.title)+'</div></div><button class="btn btn-ghost btn-xs" onclick="event.stopPropagation();removeWatchItem(\''+m.id+'\')">🗑</button></div>'})}
  else{html+='<div class="empty"><div class="empty-icon">🎵</div><div class="empty-title">Пусто</div></div>'}
  html+='</div>';
  document.getElementById('app').innerHTML=html;
}
function renderGames(){
  var list=state.watchlist.filter(function(w){return w.type==='game'})||[];
  var html='<div class="page"><div class="row-between" style="margin-bottom:14px;"><div class="title-xl" style="margin:0;">🎮 Игры</div><button class="btn btn-primary btn-sm" onclick="addWatchItem(\'game\')">+</button></div>';
  if(list.length){list.forEach(function(m){html+='<div class="list-row"><div class="list-icon">🎮</div><div class="list-body"><div class="list-title">'+esc(m.title)+'</div></div><button class="btn btn-ghost btn-xs" onclick="event.stopPropagation();removeWatchItem(\''+m.id+'\')">🗑</button></div>'})}
  else{html+='<div class="empty"><div class="empty-icon">🎮</div><div class="empty-title">Пусто</div></div>'}
  html+='</div>';
  document.getElementById('app').innerHTML=html;
...
function renderPodcasts(){
  var list=state.watchlist.filter(function(w){return w.type==='podcast'})||[];
  var html='<div class="page"><div class="row-between" style="margin-bottom:14px;"><div class="title-xl" style="margin:0;">🎧 Подкасты</div><button class="btn btn-primary btn-sm" onclick="addWatchItem(\'podcast\')">+</button></div>';
  if(list.length){list.forEach(function(m){html+='<div class="list-row"><div class="list-icon">🎧</div><div class="list-body"><div class="list-title">'+esc(m.title)+'</div></div><button class="btn btn-ghost btn-xs" onclick="event.stopPropagation();removeWatchItem(\''+m.id+'\')">🗑</button></div>'})}
  else{html+='<div class="empty"><div class="empty-icon">🎧</div><div class="empty-title">Пусто</div></div>'}
  html+='</div>';
  document.getElementById('app').innerHTML=html;
...
function addWatchItem(type){
  ...
  if(!title)return;
  var rating=prompt('Оценка (1-10, необязательно):','');
  state.watchlist.push({id:uid(),type:type,title:title,rating:rating?parseInt(rating):null,added:nowISO()});
  save();toast('✓ Добавлено','success');
  if(type==='movie')renderMovies();else if(type==='series')renderSeries();else if(type==='book')renderBooks();
  else if(type==='music')renderMusic();else if(type==='game')renderGames();else if(type==='podcast')renderPodcasts();
}
...
function renderFocus(){
  ...
  if(sessions.length){
    ...
    sessions.slice(-5).reverse().forEach(function(s){
      html+='<div class="stat-row"><span class="stat-row-label">'+s.name+'</span><span class="stat-row-value">'+s.duration+' мин · '+s.date+'</span></div>';
    });
    html+='</div>';
  }
  html+='</div>';
...
function renderMore(){
  var groups=[
    {title:'🎓 Обучение',items:[
      ...
      {key:'memory',emoji:'🧠',label:'Память'},
      {key:'iq',emoji:'🎯',label:'IQ'},
      {key:'eq',emoji:'❤️',label:'EQ'},
      {key:'finance',emoji:'💰',label:'Финансы'},
      {key:'neuromodule',emoji:'🔬',label:'Нейро'}
    ]},
...
/* ============================================================
   ...
   ============================================================ */
var MED_METRICS=[
{id:'bp_sys',name:'Давление систолическое',unit:'мм рт.ст.',icon:'💓',normal:[110,130],warn:[90,140],danger:[80,180]},
{id:'bp_dia',name:'Давление диастолическое',unit:'мм рт.ст.',icon:'💓',normal:[70,85],warn:[60,90],danger:[50,110]},
...
{id:'height',name:'Рост',unit:'см',icon:'📏',normal:[0,250],warn:[0,250],danger:[0,250]},
...
{id:'sleep',name:'Сон',unit:'часов',icon:'😴',normal:[7,9],warn:[6,10],danger:[4,12]},
...
{id:'hgb',name:'Гемоглобин',unit:'г/л',icon:'🩸',normal:[120,160],warn:[100,180],danger:[70,200]},
{id:'chol',name:'Холестерин',unit:'ммоль/л',icon:'🫀',normal:[3.0,5.2],warn:[2.5,6.2],danger:[2.0,8.0]}
];
...
function getStatusColor(status){return{normal:'var(--success)',warn:'var(--warning)',danger:'var(--danger)',unknown:'var(--text-3)'}[status]||'var(--text-3)'}
function getStatusLabel(status){return{normal:'Норма',warn:'Внимание',danger:'Опасно',unknown:'—'}[status]||'—'}
...
function renderMedical(){
  ...
  html+='<div class="card card-gradient" style="text-align:center;"><div style="font-size:40px;">⚕️</div><div style="font-size:18px;font-weight:800;margin-top:6px;">ИИ-Врач</div><div style="font-size:12px;opacity:.9;margin-top:4px;">Анализ · рекомендации · тренды</div></div>';
  html+='<div class="card"><h2>👤 Профиль пациента</h2>';
  var p=data.profile||{};
  html+='<div class="list-row" onclick="openMedicalProfile()"><div class="list-icon">📋</div><div class="list-body"><div class="list-title">'+(p.age?p.age+' лет':'Возраст не указан')+' · '+(p.sex==='m'?'Мужской':p.sex==='f'?'Женский':'Пол не указан')+'</div><div class="list-subtitle">'+(p.weight?p.weight+' кг · ':'')+(p.height?p.height+' см':'')+(p.chronic&&p.chronic.length?' · '+p.chronic.join(', '):'')+'</div></div><div class="list-chevron">›</div></div>';
  html+='</div>';
  ...
  MED_METRICS.forEach(function(m){
    ...
    if(entries.length){
      ...
      html+='<div class="list-row" onclick="openMedMetricDetail(\''+m.id+'\')" style="border-left:3px solid '+color+';"><div class="list-icon">'+m.icon+'</div><div class="list-body"><div class="list-title">'+m.name+'</div><div class="list-subtitle"><span style="color:'+color+';font-weight:700;">'+last.value+' '+m.unit+'</span> · '+getStatusLabel(status)+' · '+last.date+'</div></div><div class="list-chevron">›</div></div>';
    }
  });
  ...
  html+='<div class="card" style="background:rgba(255,107,107,.08);border-color:rgba(255,107,107,.3);"><h2>⚠️ Дисклеймер</h2><div class="footnote text-secondary">ИИ-Врач не заменяет реального врача. При острых симптомах — 103/112.</div></div>';
  html+='</div>';
  document.getElementById('app').innerHTML=html;
}
function openMedicalProfile(){
  ...
  html+='<div class="field"><label class="field-label">Хронические (через запятую)</label><input type="text" id="med-chronic" value="'+esc((p.chronic||[]).join(', '))+'"/></div>';
...
function saveMedicalProfile(){
  ...
  var cs=(document.getElementById('med-chronic')||{}).value||'';
  state.medicalData.profile.chronic=cs.split(',').map(function(s){return s.trim()}).filter(function(s){return s});
...
{value:120,label:'За 2 часа'},{value:1440,label:'За 1 день'},{value:10080,label:'За 1 неделю'}
];
var CAL_REPEATS=[
...
function renderGcal(){
  ...
  html+='</div></div>';
  if(gcal.connected){
    html+='<div class="card" style="background:rgba(61,220,151,.1);border-color:rgba(61,220,151,.35);padding:10px 14px;"><div class="row-between"><div class="footnote" style="color:var(--success);font-weight:700;">✓ Google Calendar подключён</div><button class="btn btn-ghost btn-xs" onclick="syncAllToGoogle()">🔄</button></div></div>';
  }else{
    html+='<div class="card" style="background:rgba(91,158,255,.08);border-color:rgba(91,158,255,.3);padding:10px 14px;"><div class="footnote text-secondary mb-2">📅 Подключи Google Calendar</div><button class="btn btn-primary btn-sm" onclick="openGcalSettings()">Подключить</button></div>';
  }
...
function calRenderWeek(){
  var start=calStartOfWeek(CAL_CURSOR);var todayISO=today();
  ...
  for(var i=0;i<7;i++){
    ...
    if(evs.length){
      ...
      evs.forEach(function(ev){
        ...
        html+='<div class="cal-week-event-title">'+esc(ev.title||'Событие')+'</div>';
        html+='</div>';
      });
      html+='</div>';
...
function calRenderEventCard(ev){
  ...
  html+='<div class="cal-event-card-title">'+esc(ev.title||'Событие')+'</div>';
  if(ev.description)html+='<div class="cal-event-card-desc">'+esc(ev.description).slice(0,100)+'</div>';
  var badges=[];
...
function openEventEditor(id){
  ...
  html+='</select></div>';
  var gcal=state.integrations.gcal||{};
  if(gcal.connected)html+='<div class="card" style="background:rgba(61,220,151,.1);border-color:rgba(61,220,151,.3);padding:10px;margin-bottom:10px;"><label style="display:flex;align-items:center;gap:10px;font-size:13px;"><input type="checkbox" id="cev-google" checked style="width:20px;height:20px;"/> ☁️ В Google Calendar</label></div>';
...
async function saveEvent(id){
  ...
  if(toGoogle&&(state.integrations.gcal||{}).connected){
    try{
      ...
      if(gid){savedEv.googleEventId=gid;save();toast('☁️ В Google','success');}
...
function deleteEvent(id){
  ...
  if(ev&&ev.googleEventId)deleteGoogleEvent(ev.googleEventId).catch(function(){});
...
function googleExportEvent(id){
  var ev=(state.calendarEvents||[]).find(function(x){return x.id===id});if(!ev)return;
  window.open(calBuildGoogleUrl(ev),'_blank');
}
...
function openGcalSettings(){
  ...
  var html='<div class="card" style="background:rgba(91,158,255,.08);border-color:rgba(91,158,255,.3);"><div class="footnote text-secondary mb-3">Для автосоздания событий нужен Google OAuth Client ID.</div><div class="footnote text-secondary">Как получить:<br>1. console.cloud.google.com<br>2. Google Calendar API → Enable<br>3. OAuth Client ID (Web)<br>4. Authorized origins: домен приложения</div></div>';
  html+='<div class="field"><label class="field-label">Client ID</label><input type="text" id="gcal-client" value="'+esc(g.clientId||'')+'"/></div>';
  html+='<button class="btn btn-primary btn-block" onclick="saveGcalClient()">💾 Сохранить</button>';
  if(g.connected){
    html+='<button class="btn btn-ghost btn-block mt-2" onclick="syncAllToGoogle()">🔄 Синхронизировать</button>';
  ...
  html+='<div class="footnote text-tertiary mt-3">Без OAuth события открываются через «📤 Открыть в Google» — работает всегда.</div>';
...
function saveGcalClient(){
  ...
  save();toast('✓ Сохранён','success');closeSheet();
}
function startGcalOAuth(){
  ...
  window.location.href='https://accounts.google.com/o/oauth2/v2/auth?client_id='+encodeURIComponent(g.clientId)+'&redirect_uri='+encodeURIComponent(redirect)+'&response_type=token&scope='+encodeURIComponent(scope)+'&include_granted_scopes=true&prompt=consent';
}
function checkGcalCallback(){
  if(location.hash&&location.hash.indexOf('access_token')>=0){
    ...
    if(params.access_token){
      ...
      toast('✓ Google подключён','success',3000);
...
async function createGoogleEvent(ev){
  ...
  var resp=await fetch('https://www.googleapis.com/calendar/v3/calendars/primary/events',{method:'POST',headers:{'Authorization':'Bearer '+g.accessToken,'Content-Type':'application/json'},body:JSON.stringify(body)});
...
async function deleteGoogleEvent(googleId){
  ...
  await fetch('https://www.googleapis.com/calendar/v3/calendars/primary/events/'+googleId,{method:'DELETE',headers:{'Authorization':'Bearer '+g.accessToken}});
}
async function syncAllToGoogle(){
  var g=state.integrations.gcal||{};
  if(!g.connected){toast('Сначала подключи Google','warning');return}
  var events=state.calendarEvents||[];var synced=0;
  for(var i=0;i<events.length;i++){
    var ev=events[i];
    if(ev.googleEventId)continue;
    try{var gid=await createGoogleEvent(ev);if(gid){ev.googleEventId=gid;synced++}}catch(e){}
  }
  save();toast('☁️ Синхронизировано: '+synced,'success');renderGcal();
}
function disconnectGcal(){
  if(!confirm('Отключить?'))return;
...
window.checkGcalCallback=checkGcalCallback;
...
function screenGetWeek(){
  ...
  for(var i=0;i<7;i++){
    ...
    total+=(state.screenStats||{})[d.toISOString().slice(0,10)]||0;
  }
  return total;
}
...
function openScreenAdd(){
  ...
  html+='<div class="field"><label class="field-label">Минут</label><input type="number" id="scr-mins" placeholder="120" min="0" step="15"/></div>';
...
function getCurrentDay(){
  ...
  return Math.min(30,Math.max.apply(null,completed)+1);
}
...
function renderPlanToday(){
  ...
  html+='<button class="btn btn-primary btn-block" onclick="createDayPlan(\''+date+'\')">'+(plan?'Обновить':'Создать')+'</button>';
  html+='</div>';
  document.getElementById('app').innerHTML=html;
}
...
function createWeekPlan(weekKey){
  ...
  save();toast('✓ Сохранено','success');renderPlanWeek();
}
...
function createMonthPlan(monthKey){
  ...
  save();toast('✓ Сохранено','success');renderPlanMonth();
}
function getWeekKey(){
...
function saveObsidian(){
  ...
  save();toast('✓ Сохранён','success');renderObsidian();
}
...
function fallbackReply(persona,text){
  ...
  if(persona==='nutrition')return 'Расскажи про питание.';
  if(persona==='fitness')return 'Какой уровень подготовки?';
...
function renderSettings(){
  ...
  if(pwaInstallPrompt){
    html+='<div class="card" style="background:linear-gradient(135deg,color-mix(in srgb,var(--brand) 15%,transparent),color-mix(in srgb,var(--brand-2) 10%,transparent));"><h2>📱 Установить</h2><div class="footnote text-secondary mb-3">Работает без Telegram.</div><button class="btn btn-primary btn-block" onclick="installPWA()">Установить</button></div>';
  }
...
function renderStorage(){
  ...
  html+='<div class="card"><h2>🔒 Правила сохранения</h2><div class="footnote text-secondary">1. Ключ не меняется никогда<br>2. Миграция без потери данных<br>3. Бэкап перед загрузкой<br>4. Сброс только с подтверждением</div></div>';
  html+='<div class="card"><h2>Основные данные</h2>';
  ...
  html+='<button class="btn btn-ghost btn-block mb-2" onclick="restoreBackup()">🔄 Восстановить из бэкапа</button>';
  html+='<button class="btn btn-danger btn-block" onclick="resetAllWithConfirm()">🗑 Полный сброс (с бэкапом)</button>';
  html+='</div>';
  html+='</div>';
  document.getElementById('app').innerHTML=html;
}
...
function saveGemini(){
  ...
  save();toast('✓ Сохранён','success');renderIntegrations();
}
