'use strict';
/* ============================================================
   LIFE OS — APP.js (ВСЁ В ОДНОМ)
   Логика + Календарь + Медицина + Все рендеры
   ============================================================ */
...
try{
  ...
  if(raw){
    ...
    state.medicalData=Object.assign({metrics:[],entries:[]},parsed.medicalData||{});
...
var PAGE_PARENTS={
  ...
  obsidian:'planning',gcal:'planning',
...
var PAGE_TITLES={
  ...
  planmonth:'План месяца',obsidian:'Obsidian',gcal:'Календарь',
  ...
  dailySurvey:'Опрос',survey:'Опрос',plan:'План',calendar:'Календарь'
};
...
function openHormone(id){
  ...
  var html='<div style="text-align:center;margin-bottom:16px;"><div style="font-size:48px;">'+h.emoji+'</div><div style="font-size:20px;font-weight:800;">'+h.name+'</div><div class="footnote text-secondary">'+h.role+'</div></div>';
  html+='<div class="card"><div class="lesson-section practice"><div class="lesson-section-title">📈 Как повысить</div><div class="lesson-content"><ul>'+h.up.map(function(x){return '<li>'+esc(x)+'</li>'}).join('')+'</ul></div></div></div>';
  html+='<div class="card"><div class="lesson-section theory"><div class="lesson-section-title">📉 Чего избегать</div><div class="lesson-content"><ul>'+h.down.map(function(x){return '<li>'+esc(x)+'</li>'}).join('')+'</ul></div></div></div>';
  ...
  html+='<div class="card" style="background:rgba(255,107,107,.08);border-color:rgba(255,107,107,.3);"><h2>⚠️ Дисбаланс</h2><div class="footnote text-secondary">'+esc(h.imbalance)+'</div></div>';
  html+='<div class="card"><h2>🎯 Протокол</h2><ul>'+h.protocol.map(function(x){return '<li>'+esc(x)+'</li>'}).join('')+'</ul></div>';
...
function renderMedical(){
  var data=state.medicalData||{metrics:[],entries:[]};
  var html='<div class="page"><div class="title-xl">🏥 Медицина</div>';
  html+='<div class="card card-gradient"><div style="text-align:center;font-size:36px;margin-bottom:6px;">⚕️</div><div style="text-align:center;font-size:16px;font-weight:800;">ИИ-Врач</div></div>';
  html+='<button class="btn btn-primary btn-block" onclick="openMedAddEntry()">➕ Добавить замер</button>';
  html+='<div class="card" style="margin-top:14px;"><h2>📊 Последние</h2>';
  var hasAny=false;
  MED_METRICS.forEach(function(m){
    var last=data.entries.filter(function(e){return e.metricId===m.id}).pop();
    if(last){
      hasAny=true;
      html+='<div class="list-row" onclick="openMedMetricDetail(\''+m.id+'\')"><div class="list-icon">'+m.icon+'</div><div class="list-body"><div class="list-title">'+m.name+'</div><div class="list-subtitle">'+last.value+' '+m.unit+' · '+last.date+'</div></div></div>';
    }
  });
...
function medSaveEntry(){
  ...
  if(!state.medicalData)state.medicalData={metrics:[],entries:[]};
...
function openMedMetricDetail(id){
  ...
  if(list.length){
    ...
    list.slice(0,50).forEach(function(e){
      html+='<div class="stat-row"><span class="stat-row-label">'+e.date+'</span><span class="stat-row-value">'+e.value+' '+m.unit+'</span></div>';
    });
    html+='</div>';
  }
...
function calEscape(s){return String(s==null?'':s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#39;')}
...
function openEventEditor(id){
  ...
  var html='';
  html+='<div class="field"><label class="field-label">Название</label><input type="text" id="cev-title" value="'+calEscape(title)+'" placeholder="Встреча, задача..."/></div>';
  html+='<div class="field"><label class="field-label">Описание</label><textarea id="cev-desc">'+calEscape(description)+'</textarea></div>';
  html+='<div class="field"><label class="field-label">Начало</label><input type="datetime-local" id="cev-start" value="'+start+'"/></div>';
  html+='<div class="field"><label class="field-label">Конец <span style="text-transform:none;font-weight:500;color:var(--text-3);">(необязательно)</span></label><input type="datetime-local" id="cev-end" value="'+end+'"/></div>';
...
function saveEvent(id){
  ...
  if(id){
    ...
    state.calendarEvents.push({id:uid(),title:title.trim(),description:description,start:start,end:end,color:color,notify:notify,repeat:repeat,createdAt:nowISO()});
  }
  save();haptic('success');closeSheet();
...
function renderSettings(){
  ...
  html+='<div class="footnote text-tertiary" style="text-align:center;margin-top:24px;">Life OS v40</div></div>';
  document.getElementById('app').innerHTML=html;
}
function renderStorage(){
  ...
  html+='<div class="card"><h2>Операции</h2><button class="btn btn-primary btn-block mb-2" onclick="exportDB()">📤 Экспорт</button><button class="btn btn-danger btn-block" onclick="if(confirm(\'Сброс?\')){localStorage.clear();location.reload();}">🗑 Полный сброс</button></div>';
  html+='</div>';
  document.getElementById('app').innerHTML=html;
}
function exportDB(){
...
function showWelcome(){
  ...
  overlay.innerHTML='<div class="welcome-logo">🧠</div><div class="welcome-title">Life OS</div><div class="welcome-sub">Нейроэкосистема жизни. Обучение, планирование, зрение, AI.</div><button class="btn btn-primary btn-block" style="max-width:340px;" onclick="startOnboarding()">Начать</button>';
  document.body.appendChild(overlay);
}
