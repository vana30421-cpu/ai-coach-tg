...
function startEffects(){
  ...
  }else if(effect==='dust'||effect==='fireflies'){
...
function renderLearning(){
  ...
  html+='<div class="compact-item" onclick="navigate(\'courses\')"><span class="compact-icon">📖</span><span>Курсы '+(COURSES_LIBRARY||[]).length+'</span></div>';
  html+='<div class="compact-item" onclick="navigate(\'paths\')"><span class="compact-icon">🗺</span><span>Пути '+(PATHS_LIBRARY||[]).length+'</span></div>';
...
function openCourse(id){
  ...
  html+='<div style="text-align:center;margin-bottom:16px;"><div style="font-size:56px;">'+c.emoji+'</div><div class="title-xl">'+c.title+'</div><div class="footnote text-secondary">'+c.category+' · '+c.hours+' ч</div></div>';
  html+='<div class="card"><div class="progress"><div class="progress-fill" style="width:'+progress+'%;"></div></div></div>';
  html+='<div class="card"><h2>📚 Уроки</h2>';
  c.lessons.forEach(function(lesson,i){
    var isDone=completed.indexOf(i)>=0;
...
function renderProfile(){
  ...
  html+='<div class="profile-hero" style="text-align:center;padding:24px;background:var(--glass-1);border:1px solid var(--glass-border);border-radius:24px;margin-bottom:16px;"><div class="avatar-btn lvl-'+Math.min(lvl,5)+'" onclick="pickEmoji()" style="width:96px;height:96px;margin:0 auto 12px;font-size:48px;">'+p.emoji+'</div><div style="font-size:22px;font-weight:800;">'+esc(p.name||'Пользователь')+'</div></div>';
...
function renderIntegrations(){
  ...
  html+='<div class="card"><h2>Gemini AI</h2><div class="field"><label class="field-label">API Key</label><input type="password" id="gem-key" value="'+esc((state.integrations.gemini||{}).apiKey||'')+'"/></div><button class="btn btn-primary btn-block" onclick="saveGemini()">💾</button></div>';
  html+='</div>';
  document.getElementById('app').innerHTML=html;
}
function saveGemini(){
...
function renderRecovery(){placeholderPage('🌿','Восстановление')}
...
function showWelcome(){
  ...
  overlay.innerHTML='<div class="welcome-logo">🧠</div><div class="welcome-title">AI Health v33</div><div class="welcome-sub">5 уровней · 250+ уроков · 150+ навыков · 50 курсов · 22 пути · Детокс · AI.</div><button class="btn btn-primary btn-block" style="max-width:340px;" onclick="startOnboarding()">Начать</button>';
  document.body.appendChild(overlay);
}
