'use strict';
/* AI HEALTH v30 — ГЛАВНЫЙ ФАЙЛ */

try{
  var tg=window.Telegram&&window.Telegram.WebApp;
  if(tg){tg.expand();tg.ready();try{tg.setHeaderColor('#000000')}catch(e){}
...
function toast(msg,type,duration){
  ...
  c.appendChild(el);
  setTimeout(function(){
    el.style.opacity='0';el.style.transform='translate(-50%,-16px)';
...
var audioCtx=null;
function initAudio(){if(!audioCtx){try{audioCtx=new AudioCtx()}catch(e){}}return audioCtx}
...
function playClickSound(){playTone(800,0.05,'sine',0.08)}
function playSuccessSound(){playTone(523,0.1,'sine',0.12);setTimeout(function(){playTone(659,0.1,'sine',0.12)},100);setTimeout(function(){playTone(784,0.2,'sine',0.12)},200)}
function playErrorSound(){playTone(200,0.3,'sawtooth',0.1)}
function playAchievementSound(){playTone(523,0.15,'sine',0.15);setTimeout(function(){playTone(659,0.15,'sine',0.15)},150);setTimeout(function(){playTone(784,0.15,'sine',0.15)},300);setTimeout(function(){playTone(1047,0.4,'sine',0.15)},450)}
function playWaterSound(){playTone(600,0.08,'sine',0.1);setTimeout(function(){playTone(800,0.08,'sine',0.1)},80)}
function playLevelUpSound(){playTone(392,0.15,'sine',0.15);setTimeout(function(){playTone(523,0.15,'sine',0.15)},150);setTimeout(function(){playTone(659,0.15,'sine',0.15)},300);setTimeout(function(){playTone(784,0.5,'sine',0.15)},450)}
...
function closeSheet(){document.querySelectorAll('.overlay').forEach(function(o){o.remove()});document.body.style.overflow='';playClickSound()}
...
function openLiveControl(){
  ...
  var items=[
    ...
    {icon:'💎',label:'Навыки (122)',action:'navigate(\'skills\');closeLiveControl()'},
    ...
    {icon:'📱',label:'Детокс',action:'navigate(\'screentracker\');closeLiveControl()'},
...
function openThemePicker(){
  ...
  html+='</div>';
  html+='<div class="card" style="margin:0;padding:16px;"><div class="row-between mb-3"><div><div class="list-title">🌈 Живые эффекты</div><div class="caption text-tertiary">Дым, волны, снег</div></div><button class="switch '+(state.settings.effectsEnabled?'on':'')+'" onclick="state.settings.effectsEnabled=!state.settings.effectsEnabled;save();startEffects();toast(\'Ок\',\'success\');"></button></div>';
...
function startEffects(){
  ...
  if(effect==='stars'){
    for(var i=0;i<cnt(15);i++){var s=document.createElement('div');s.className='effect-star';
      s.style.left=Math.random()*100+'%';s.style.top=Math.random()*100+'%';s.style.animationDelay=Math.random()*3+'s';overlay.appendChild(s)}
  } else if(effect==='rain'){
    for(var j=0;j<cnt(25);j++){var d=document.createElement('div');d.className='effect-drop';
      d.style.left=Math.random()*100+'%';d.style.animationDuration=((Math.random()*1.5+1.5)/speed)+'s';d.style.animationDelay=Math.random()*5+'s';d.style.height=(Math.random()*20+12)+'px';overlay.appendChild(d)}
  } else if(effect==='petals'){
    for(var k=0;k<cnt(12);k++){var p=document.createElement('div');p.className='effect-petal';p.textContent='🌸';
      p.style.left=Math.random()*100+'%';p.style.animationDuration=((Math.random()*8+10)/speed)+'s';p.style.animationDelay=Math.random()*10+'s';overlay.appendChild(p)}
  } else if(effect==='leaves'){
    for(var l=0;l<cnt(10);l++){var lf=document.createElement('div');lf.className='effect-leaf';lf.textContent='🍃';
      lf.style.left=Math.random()*100+'%';lf.style.animationDuration=((Math.random()*8+8)/speed)+'s';lf.style.animationDelay=Math.random()*10+'s';overlay.appendChild(lf)}
  } else if(effect==='snow'){
    for(var n=0;n<cnt(18);n++){var sn=document.createElement('div');sn.className='effect-snowflake';sn.textContent='❄';
      sn.style.left=Math.random()*100+'%';sn.style.animationDuration=((Math.random()*6+8)/speed)+'s';sn.style.animationDelay=Math.random()*10+'s';overlay.appendChild(sn)}
    ...
    for(var sm=0;sm<cnt(6);sm++){var smk=document.createElement('div');smk.className='effect-smoke';
      smk.style.left=Math.random()*100+'%';smk.style.animationDuration=((Math.random()*8+12)/speed)+'s';smk.style.animationDelay=Math.random()*10+'s';overlay.appendChild(smk)}
    ...
    for(var cr=0;cr<cnt(18);cr++){var dm=document.createElement('div');dm.className='effect-diamond';dm.textContent='◆';
      dm.style.left=Math.random()*100+'%';dm.style.animationDuration=((Math.random()*8+10)/speed)+'s';dm.style.animationDelay=Math.random()*10+'s';overlay.appendChild(dm)}
  } else if(effect==='bats'){
    for(var b=0;b<cnt(4);b++){var bat=document.createElement('div');bat.className='effect-fly';bat.textContent='🦇';
      bat.style.left=Math.random()*100+'%';bat.style.top=Math.random()*50+'%';bat.style.animationDelay=Math.random()*5+'s';overlay.appendChild(bat)}
  } else if(effect==='spiders'){
    for(var sp=0;sp<cnt(4);sp++){var spd=document.createElement('div');spd.className='effect-fly';spd.textContent='🕷';
      spd.style.left=Math.random()*100+'%';spd.style.top=Math.random()*50+'%';spd.style.animationDelay=Math.random()*5+'s';overlay.appendChild(spd)}
    ...
    for(var sd=0;sd<cnt(25);sd++){var sdust=document.createElement('div');sdust.className='effect-dust';
      sdust.style.left=Math.random()*100+'%';sdust.style.top=Math.random()*100+'%';sdust.style.animationDuration=((Math.random()*4+4)/speed)+'s';sdust.style.animationDelay=Math.random()*6+'s';overlay.appendChild(sdust)}
  } else if(effect==='cyber'){
    for(var cy=0;cy<cnt(12);cy++){var cyd=document.createElement('div');cyd.className='effect-code';cyd.textContent='01';
      cyd.style.left=Math.random()*100+'%';cyd.style.animationDuration=((Math.random()*4+4)/speed)+'s';cyd.style.animationDelay=Math.random()*5+'s';overlay.appendChild(cyd)}
  } else if(effect==='ember'||effect==='fire'){
    for(var em=0;em<cnt(25);em++){var ember=document.createElement('div');ember.className='effect-ember';
      ember.style.left=Math.random()*100+'%';ember.style.bottom=(Math.random()*100)+'px';ember.style.animationDuration=((Math.random()*3+3)/speed)+'s';ember.style.animationDelay=Math.random()*4+'s';overlay.appendChild(ember)}
  } else if(effect==='bubbles'){
    for(var bb=0;bb<cnt(15);bb++){var bub=document.createElement('div');bub.className='effect-bubble';
      bub.style.left=Math.random()*100+'%';var size=Math.random()*20+8;bub.style.width=size+'px';bub.style.height=size+'px';bub.style.animationDuration=((Math.random()*8+6)/speed)+'s';bub.style.animationDelay=Math.random()*8+'s';overlay.appendChild(bub)}
  } else if(effect==='neon'||effect==='holo'){
    for(var nn=0;nn<cnt(25);nn++){var spark=document.createElement('div');spark.className='effect-spark';
      spark.style.left=Math.random()*100+'%';spark.style.top=Math.random()*100+'%';spark.style.animationDelay=Math.random()*3+'s';
      if(effect==='holo'){spark.style.background='linear-gradient(45deg,#00ffcc,#ff00aa)'}
    ...
    for(var gl=0;gl<cnt(3);gl++){var gal=document.createElement('div');gal.className='effect-galaxy';
      gal.style.width='250px';gal.style.height='250px';gal.style.left=(Math.random()*80)+'%';gal.style.top=(Math.random()*80)+'%';gal.style.animationDelay=(gl*5)+'s';overlay.appendChild(gal)}
  } else if(effect==='nebula'){
    for(var nb=0;nb<cnt(3);nb++){var neb=document.createElement('div');neb.className='effect-nebula';
      neb.style.width='350px';neb.style.height='350px';neb.style.left=(Math.random()*70)+'%';neb.style.top=(Math.random()*70)+'%';neb.style.animationDelay=(nb*4)+'s';overlay.appendChild(neb)}
  } else if(effect==='code'){
    for(var cd=0;cd<cnt(10);cd++){var code=document.createElement('div');code.className='effect-code';code.textContent=Math.random()>0.5?'1':'0';
      code.style.left=Math.random()*100+'%';code.style.animationDuration=((Math.random()*4+4)/speed)+'s';code.style.animationDelay=Math.random()*5+'s';overlay.appendChild(code)}
  } else if(effect==='spark'){
    for(var sk=0;sk<cnt(30);sk++){var spk=document.createElement('div');spk.className='effect-spark';
      spk.style.left=Math.random()*100+'%';spk.style.top=Math.random()*100+'%';spk.style.animationDelay=Math.random()*2+'s';overlay.appendChild(spk)}
  } else if(effect==='stardust'){
    for(var std=0;std<cnt(45);std++){var stdust=document.createElement('div');stdust.className='effect-stardust';
      stdust.style.left=Math.random()*100+'%';stdust.style.top=Math.random()*100+'%';stdust.style.animationDelay=Math.random()*3+'s';overlay.appendChild(stdust)}
  } else if(effect==='hearts'){
    for(var hr=0;hr<cnt(12);hr++){var ht=document.createElement('div');ht.className='effect-petal';ht.textContent='❤';
      ht.style.left=Math.random()*100+'%';ht.style.animationDuration=((Math.random()*8+8)/speed)+'s';ht.style.animationDelay=Math.random()*10+'s';overlay.appendChild(ht)}
  } else if(effect==='fireflies'){
    for(var ff=0;ff<cnt(15);ff++){var ffl=document.createElement('div');ffl.className='effect-dust';
      ffl.style.left=Math.random()*100+'%';ffl.style.top=Math.random()*100+'%';ffl.style.background='#ffcc4d';ffl.style.boxShadow='0 0 12px #ffcc4d,0 0 24px #ffa940';ffl.style.animationDuration=((Math.random()*4+4)/speed)+'s';ffl.style.animationDelay=Math.random()*6+'s';overlay.appendChild(ffl)}
  } else if(effect==='particles'){
    for(var pt=0;pt<cnt(20);pt++){var part=document.createElement('div');part.className='effect-stardust';
      part.style.left=Math.random()*100+'%';part.style.top=Math.random()*100+'%';part.style.background='var(--brand)';part.style.boxShadow='0 0 8px var(--brand)';part.style.animationDelay=Math.random()*3+'s';overlay.appendChild(part)}
  }
}
...
function getLevelProgress(levelId){
  ...
  var total=0,done=0;
  level.modules.forEach(function(mod){mod.lessons.forEach(function(l,idx){total++;if(state.levelProgress&&state.levelProgress[levelId+'_'+mod.id+'_'+idx])done++})});
  return{done:done,total:total,pct:total>0?Math.round(done/total*100):0}
}
function isLevelUnlocked(levelId){var idx=LEARNING_LEVELS.findIndex(function(l){return l.id===levelId});if(idx<=0)return true;var prev=LEARNING_LEVELS[idx-1];return getLevelProgress(prev.id).pct===100}
function isModuleUnlocked(level,moduleIdx){if(moduleIdx===0)return true;var prevMod=level.modules[moduleIdx-1];return prevMod.lessons.every(function(l,idx){return state.levelProgress[level.id+'_'+prevMod.id+'_'+idx]})}
function isLessonUnlocked(level,module,lessonIdx){if(lessonIdx===0)return true;var key=level.id+'_'+module.id+'_'+(lessonIdx-1);return !!state.levelProgress[key]}
...
function renderDashboard(){
  ...
  var waterEntry=state.customWater.find(function(w){return w.date===today()})||state.water.find(function(w){return w.date===today()});
  ...
  html+='<div class="card card-gradient"><div class="row-between" style="margin-bottom:14px;"><div><div style="font-size:13px;opacity:.85;font-weight:600;margin-bottom:6px;">Прогресс дня</div><div style="font-size:40px;font-weight:800;line-height:1;">'+doneToday+'/'+todayTasks.length+'</div></div><div style="font-size:48px;font-weight:800;">'+progress+'%</div></div><div class="progress" style="background:rgba(255,255,255,.25);height:8px;"><div class="progress-fill" style="width:'+progress+'%;background:#fff;"></div></div></div>';
  html+='<div class="stat-grid mb-4"><div class="stat-item"><div class="stat-value">'+(state.stats.streak||0)+'</div><div class="stat-label">Streak</div></div><div class="stat-item"><div class="stat-value">'+overallPct+'%</div><div class="stat-label">Обучение</div></div><div class="stat-item"><div class="stat-value">'+skillsDone+'/'+skillsTotal+'</div><div class="stat-label">Навыки</div></div></div>';
  ...
  html+='<div class="card" style="background:linear-gradient(135deg,rgba(167,139,250,.15),rgba(91,158,255,.1));border-color:rgba(167,139,250,.3);"><h2>🎓 Обучение</h2><div class="footnote text-secondary" style="margin-bottom:12px;">5 уровней + 5 модулей</div><button class="btn btn-primary btn-block" onclick="navigate(\'learning\')">Открыть</button></div>';
  html+='<div class="card" style="background:linear-gradient(135deg,rgba(255,204,77,.15),rgba(255,169,64,.1));border-color:rgba(255,204,77,.3);"><h2>💎 Навыки (122)</h2><div class="footnote text-secondary" style="margin-bottom:12px;">15 категорий</div><button class="btn btn-primary btn-block" onclick="navigate(\'skills\')">Открыть</button></div>';
  html+='<div class="card" style="background:linear-gradient(135deg,rgba(60,220,130,.15),rgba(91,158,255,.1));border-color:rgba(60,220,130,.3);"><h2>🇬🇧 English 125</h2><button class="btn btn-primary btn-block" onclick="navigate(\'english\')">Продолжить</button></div>';
  html+='<div class="card" style="background:linear-gradient(135deg,rgba(255,123,169,.15),rgba(255,169,64,.1));border-color:rgba(255,123,169,.3);"><h2>🎬 Досуг</h2><button class="btn btn-primary btn-block" onclick="navigate(\'entertainment\')">Открыть</button></div>';
  html+='<div class="card" style="background:linear-gradient(135deg,rgba(255,107,107,.15),rgba(255,169,64,.1));border-color:rgba(255,107,107,.3);"><h2>📱 Детокс</h2><button class="btn btn-primary btn-block" onclick="navigate(\'screentracker\')">Открыть</button></div>';
  if(!state.profile.surveyDone){html+='<div class="card" style="background:linear-gradient(135deg,rgba(255,169,64,.15),rgba(255,107,107,.1));border-color:rgba(255,169,64,.3);"><h2>📋 Заполни профиль</h2><button class="btn btn-primary btn-block" onclick="startSurvey()">Начать</button></div>'}
  else{html+='<div class="card" style="background:linear-gradient(135deg,rgba(61,220,151,.15),rgba(91,158,255,.1));border-color:rgba(61,220,151,.3);"><h2>🎯 Персональный план</h2><button class="btn btn-primary btn-block" onclick="navigate(\'plan\')">Открыть</button></div>'}
  html+='<div class="insight-card"><div class="insight-title">💎 Мудрость дня</div><div class="insight-text">"Ты не ленивый. Ты либо устал, либо не видишь смысла, либо боишься."</div></div>';
...
function toggleTask(id){
  ...
  save();
  if(t.status==='completed'){playSuccessSound();toast('✓ Выполнено','success')}else{playClickSound()}
...
function saveEntity(type,id){
  ...
  save();playSuccessSound();haptic('success');closeSheet();
...
function deleteEntity(type,id){
  ...
  save();playErrorSound();closeSheet();toast('Удалено','info');
  if(currentPage==='tasks')renderTasks();
  else if(currentPage==='habits')renderHabits();
  else if(currentPage==='goals')renderGoals();
  else if(currentPage==='notes')renderNotes();
...
function renderLevelDetail(){
  ...
  html+='<div style="text-align:center;margin-bottom:24px;"><div style="font-size:64px;margin-bottom:12px;">'+level.emoji+'</div><div class="title-xl">Уровень '+level.num+': '+level.title+'</div></div>';
...
function renderModuleDetail(){
  ...
  html+='<div style="text-align:center;margin-bottom:24px;"><div style="font-size:56px;margin-bottom:12px;">'+module.emoji+'</div><div class="title-xl">'+module.title+'</div></div>';
  ...
  module.lessons.forEach(function(lesson,i){
    ...
    var cls='lesson-row';
    if(isDone)cls+=' done';else if(!unlocked)cls+=' locked';else cls+=' current';
...
function openLesson(idx){
  ...
  if(!isDone)html+='<button class="btn btn-primary btn-block" onclick="completeLesson()">✓ Изучено</button>';
    ...
    html+='<div class="badge badge-success" style="display:block;text-align:center;padding:14px;">✓ Изучен</div>';
...
function completeLesson(){
  ...
  save();playSuccessSound();haptic('success');toast('✓ Урок изучен!','success');
...
function completeSkill(id){
  ...
  save();playSuccessSound();haptic('success');toast('✓ Навык изучен!','success');
  checkAchievements();closeSheet();renderSkills();
}
function renderSkillDetail(){renderSkills()}

/* ============ МОДУЛИ (Memory/IQ/EQ/Finance/Neuro) ============ */
...
function completeModuleLesson(moduleKey,idx){
  ...
  save();playSuccessSound();haptic('success');toast('✓ Изучено!','success');
...
function renderEnglish(){
  ...
  var html='<div class="page">'+backBtn('dashboard')+'<div class="title-xl">🇬🇧 Английский 125</div>';
...
function renderPaths(){
  ...
  PATHS_LIBRARY.forEach(function(p){
    ...
    html+='<div class="card" style="cursor:pointer;" onclick="openPath(\''+p.id+'\')"><div style="display:flex;gap:14px;align-items:center;margin-bottom:12px;"><div style="width:56px;height:56px;border-radius:18px;background:var(--glass-2);display:grid;place-items:center;font-size:28px;flex-shrink:0;">'+p.emoji+'</div><div style="flex:1;"><div style="font-size:17px;font-weight:800;">'+p.title+'</div><div class="footnote text-secondary">'+completed+'/'+p.steps.length+' · '+p.category+'</div></div><div class="list-chevron">›</div></div><div class="progress"><div class="progress-fill" style="width:'+progress+'%;"></div></div></div>'
  });
  html+='</div>';
  document.getElementById('app').innerHTML=html;
}
...
function renderPathDetail(){
  ...
  p.steps.forEach(function(step,i){
    ...
    var cls='path-step';
    if(isDone)cls+=' completed';else if(isActive)cls+=' active';else if(isLocked)cls+=' locked';
...
function completeStep(idx){
  ...
  if(userPath.completedSteps.indexOf(idx)<0){
    ...
    playSuccessSound();toast('✓ Шаг выполнен!','success');haptic('success');
...
function completeCourseLesson(courseId,idx){
  ...
  if(userCourse.completedLessons.indexOf(idx)<0){
    ...
    playSuccessSound();toast('✓ Урок изучен!','success');haptic('success');
...
function saveDomainScore(id){
  ...
  save();playSuccessSound();toast('✓ Сохранено','success');haptic('success');
...
function renderProfile(){
  ...
  html+='<div class="profile-hero"><div class="profile-avatar" onclick="pickEmoji()">'+p.emoji+'</div><div class="profile-name">'+esc(p.name||'Пользователь')+'</div></div>';
  ...
  ACHIEVEMENTS.forEach(function(a){
    ...
    html+='<div class="achieve-item '+(isUnlocked?'unlocked':'locked')+'"><div class="achieve-icon">'+a.icon+'</div><div class="achieve-name">'+a.name+'</div></div>'
  });
  html+='</div></div></div>';
  document.getElementById('app').innerHTML=html;
}
...
function setEmoji(e){
  state.profile.emoji=e;save();playClickSound();closeSheet();toast('Обновлено','success');
  renderProfile();updateHeaderAvatar();
}
function saveProfileName(name){
  state.profile.name=(name||'').trim()||'Пользователь';
  save();playSuccessSound();toast('Сохранено','success');updateHeaderAvatar();
}
...
function renderSettings(){
  ...
  html+='<div class="footnote text-tertiary" style="text-align:center;margin-top:36px;">AI Health v30 · Vision UI</div></div>';
  document.getElementById('app').innerHTML=html;
}
...
function exportDB(){
  ...
  a.href=url;a.download='ai-health-v30-'+today()+'.json';
  a.click();URL.revokeObjectURL(url);
  toast('Экспорт готов','success');playSuccessSound();
}
function importDB(e){
  ...
  reader.onload=function(ev){
    try{
      ...
      save();toast('Импорт успешен','success');playSuccessSound();location.reload();
    }catch(err){toast('Ошибка','error');playErrorSound()}
  };
...
function renderStats(){
  ...
  var skillsDone=Object.keys(state.skillsProgress||{}).length;
  var waterTotal=(state.water||[]).reduce(function(a,w){return a+(w.count||0)},0);
...
function renderDetailedStats(){
  ...
  var skillsDone=Object.keys(state.skillsProgress||{}).length;
  var waterTotal=(state.water||[]).reduce(function(a,w){return a+(w.count||0)},0);
  var moodTotal=(state.mood||[]).length;
  var workoutTotal=(state.customWorkouts||state.workouts||[]).length;
  var meditationTotal=(state.customMeditation||state.meditations||[]).length;
...
function saveGCal(){
  ...
  playSuccessSound();toast('Сохранено','success');renderIntegrations();
}
...
function saveGemini(){
  ...
  save();playSuccessSound();toast('Gemini подключён','success');renderIntegrations();
}
...
function switchPersona(p){state.settings.activePersona=p;save();playClickSound();renderAI()}
...
function saveAISettings(){
  ...
  save();playSuccessSound();closeSheet();toast('Сохранено','success');renderAI();
}
...
function renderHealth(){
  ...
  html+='<div class="card"><h2>Сегодня</h2><div class="stat-grid"><div class="stat-item" onclick="quickMoodLog()" style="cursor:pointer;"><div class="stat-value">'+(state.mood.length?state.mood[state.mood.length-1].score+'/10':'—')+'</div><div class="stat-label">Настроение</div></div><div class="stat-item" onclick="addWater()" style="cursor:pointer;"><div class="stat-value">'+water+'/'+waterGoal+'</div><div class="stat-label">Вода</div></div><div class="stat-item" onclick="navigate(\'workouts\')" style="cursor:pointer;"><div class="stat-value">'+((state.customWorkouts||state.workouts||[]).length)+'</div><div class="stat-label">Тренировок</div></div></div></div>';
...
function openPersona(p){state.settings.activePersona=p;save();playClickSound();navigate('ai')}
function addWater(){
  var t=today();
  ...
  save();playWaterSound();toast('💧 +1','success');haptic('success');
...
function saveMood(score){
  var t=today();
  var existing=state.mood.find(function(m){return m.date===t});
  if(existing)existing.score=score;
  else state.mood.push({id:uid(),date:t,score:score,created_at:nowISO()});
  state.stats.totalMoodLogs=(state.stats.totalMoodLogs||0)+1;
  save();playSuccessSound();closeSheet();toast('Записано','success');haptic('success');
  if(currentPage==='dashboard')renderDashboard();else renderHealth();
}

/* ============ ПРОСТЫЕ РАЗДЕЛЫ ============ */
function renderWater(){
  var entry=state.customWater.find(function(w){return w.date===today()})||state.water.find(function(w){return w.date===today()});
...
function renderWorkouts(){
  var w=state.customWorkouts||state.workouts||[];
...
function renderMeditation(){
  var m=state.customMeditation||state.meditations||[];
...
function renderMeds(){
  var m=state.customMeds||state.meds||[];
...
function renderJournal(){
  ...
  if(entries.length){
    entries.slice().reverse().forEach(function(e){
      html+='<div class="card" onclick="openEntityEditor(\'journal\',\''+e.id+'\')" style="cursor:pointer;"><div class="footnote text-tertiary">'+e.date+'</div><div style="margin-top:8px;font-weight:600;">'+esc(e.title||'Запись')+'</div>';
      if(e.wins)html+='<div class="footnote text-secondary mt-2">✅ '+esc(e.wins).slice(0,100)+'</div>';
      html+='</div>'
    })
  } else {html+='<div class="empty"><div class="empty-icon">📓</div><div class="empty-title">Пусто</div></div>'}
  html+='</div>';
...
function renderNotes(){
  var notes=state.customNotes||state.notes||[];
  ...
  if(notes.length){
    notes.forEach(function(n){
      html+='<div class="card" onclick="openEntityEditor(\'note\',\''+n.id+'\')" style="cursor:pointer;"><div class="list-title">'+esc(n.title||'Без названия')+'</div><div class="footnote text-secondary mt-2">'+esc((n.content||'').slice(0,150))+'</div></div>'
    })
  } else {html+='<div class="empty"><div class="empty-icon">📝</div><div class="empty-title">Пусто</div></div>'}
  html+='</div>';
  document.getElementById('app').innerHTML=html;
...
function renderGoals(){
  var goals=state.customGoals||state.goals||[];
  ...
  if(goals.length){
    goals.forEach(function(g){
      var pct=g.target?(g.current||0)/g.target*100:0;
      html+='<div class="card" onclick="openEntityEditor(\'goal\',\''+g.id+'\')" style="cursor:pointer;"><div class="list-title">'+esc(g.title)+'</div><div class="footnote text-secondary">'+esc(g.category||'')+' '+(g.metric?'· '+g.current+'/'+g.target+' '+g.metric:'')+'</div>';
      if(g.target)html+='<div class="progress mt-2"><div class="progress-fill" style="width:'+Math.min(100,pct)+'%;"></div></div>';
      html+='</div>'
    })
...
function renderHabits(){
  ...
  if(habits.length){
    habits.forEach(function(h){
      html+='<div class="habit-row" onclick="openEntityEditor(\'habit\',\''+h.id+'\')"><div class="habit-icon">'+(h.icon||'✅')+'</div><div class="habit-body"><div class="habit-title">'+esc(h.title)+'</div><div class="habit-streak">'+esc(h.category||'')+'</div></div></div>'
    })
...
function addHabitFromTemplate(tid){
  ...
  save();playSuccessSound();toast('Привычка добавлена','success');renderHabits();
}
...
function renderTimer(){
  ...
  html+='<div class="card" style="text-align:center;padding:40px 20px;"><div style="font-size:72px;font-weight:800;line-height:1;letter-spacing:-.04em;font-variant-numeric:tabular-nums;">'+display+'</div>';
...
function startTimer(){
  ...
  timerInterval=setInterval(function(){
    if(timerSeconds<=0){pauseTimer();playSuccessSound();toast('⏰ Время вышло!','success');return}
    timerSeconds--;
    var el=document.querySelector('.card[style*="text-align:center"] div');
    if(el){
...
function finishTimer(){
  ...
  save();playSuccessSound();toast('✓ Сессия завершена','success');haptic('success');
  checkAchievements();
  resetTimer();
}
...
function startFocusSession(name,duration){
  ...
  save();playSuccessSound();toast('✓ '+name+' завершён!','success');haptic('success');
...
function renderMusic(){
  ...
  lib.forEach(function(m){
    html+='<div class="method-card"><div class="method-header"><div class="method-emoji">🎵</div><div style="flex:1;"><div class="method-title">'+esc(m.title)+'</div><div class="method-cat">'+esc(m.genre)+'</div></div></div><div class="method-desc">'+esc(m.desc)+'</div></div>'
  });
  html+='</div>';
  document.getElementById('app').innerHTML=html;
}
...
function renderPodcasts(){
  ...
  lib.forEach(function(m){
    html+='<div class="method-card"><div class="method-header"><div class="method-emoji">🎧</div><div style="flex:1;"><div class="method-title">'+esc(m.title)+'</div><div class="method-cat">'+esc(m.author)+'</div></div></div><div class="method-desc">'+esc(m.desc)+'</div></div>'
  });
  html+='</div>';
  document.getElementById('app').innerHTML=html;
}
...
function promptWatched(id){
  ...
  playSuccessSound();renderWatchlist();
}
...
function renderScreenTracker(){
  ...
  html+='<div class="stat-grid mb-4"><div class="stat-item"><div class="stat-value">'+tips.length+'</div><div class="stat-label">Советов</div></div><div class="stat-item"><div class="stat-value">'+habits.length+'</div><div class="stat-label">Привычек</div></div><div class="stat-item"><div class="stat-value">30</div><div class="stat-label">Дней план</div></div></div>';
  html+='<div class="card"><h2>🗓 30-дневный план</h2><button class="btn btn-primary btn-block" onclick="navigate(\'detoxplan\')">Открыть</button></div>';
  var categories={};
...
function renderMore(){
  var groups=[
    ...
    {title:'🎓 Обучение',items:[{key:'learning',emoji:'🎓',label:'5 Уровней + модули'},{key:'skills',emoji:'💎',label:'122 Навыка'},{key:'english',emoji:'🇬🇧',label:'Английский 125'},{key:'paths',emoji:'🗺',label:'17 Путей'},{key:'courses',emoji:'📚',label:'12 Курсов'},{key:'methods',emoji:'💎',label:'50+ Методик'}]},
    ...
    {title:'🌿 Восстановление',items:[{key:'recovery',emoji:'🌿',label:'Восстановление'},{key:'screentracker',emoji:'📱',label:'Экранный детокс'},{key:'detoxplan',emoji:'🗓',label:'30-дневный план'}]},
...
function selectSurveyOption(qid,val){
  ...
  save();playClickSound();haptic('light');nextSurveyStep();
}
function toggleSurveyMulti(qid,val){
  ...
  save();playClickSound();haptic('light');renderSurvey();
}
function saveSurveyAnswer(){
  ...
  save();playClickSound();nextSurveyStep();
}
...
function finishSurvey(){
  ...
  save();playLevelUpSound();haptic('success');
...
function showWelcome(){
  ...
  overlay.innerHTML='<div class="welcome-logo">🧠</div><div class="welcome-title">AI Health v30</div><div class="welcome-sub">5 уровней · 122 навыка · 125 уроков English · 45+ тем · Досуг · AI.</div><button class="btn btn-primary btn-block" style="max-width:340px;" onclick="startOnboarding()">Начать</button>';
  document.body.appendChild(overlay);
}
...
function finishOnboarding(){
  ...
  playLevelUpSound();haptic('success');
...
function attachRipple(){
  document.addEventListener('pointerdown',function(e){
    var target=e.target.closest('.btn, .list-row, .chip, .tab-item, .task-item, .card, .icon-btn, .avatar-btn, .persona-item, .segmented-item, .level-card, .module-card, .lesson-row, .emoji-pick, .domain-card, .method-card, .course-card, .path-step, .survey-option, .ent-card, .screen-card, .habit-row, .group-item, .back-btn, .live-panel-item');
    if(!target)return;
...
window.openSkills=function(){navigate('skills')};
