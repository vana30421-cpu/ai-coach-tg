...
function defaultState(){
  return{
    ...
    integrations:{
      ...
      gcal:{clientId:'',accessToken:null,lastSync:null,connected:false},
...
try{
  ...
  if(raw){
    ...
    ['levelProgress','englishProgress','skillsProgress','domainScores','metrics',
     'challengeProgress','screenHistory','customSleep','dailySurveys','dayPlans','weekPlans','monthPlans'].forEach(function(k){
      if(!state[k])state[k]={};
    });
    ['tasks','customHabits','customGoals','customNotes','journalEntries','customWater',
     ...
     'focusSessions','chats','paths','courses','watchlist','watched','customResources','eyeExercises'
    ].forEach(function(k){
      if(!state[k])state[k]=[];
    });
...
var currentPathId=null,currentCourseId=null,currentQuickTab='all';
...
function updateHeader(page){
  ...
  /* В новой шапке нет кнопок назад — они не нужны */
  void parent;void title;void backLabel;
}
...
function navigate(page){
  ...
  var renderers={
    ...
    visiontips:renderVisionTips,
    planning:renderPlanning,
    plantoday:renderPlanToday,
    planweek:renderPlanWeek,
...
function openLiveControl(){
  ...
  var items=[
    ...
    {icon:'👁',label:'Упражнение для глаз',action:'navigate(\'visionex\');closeLiveControl()'},
    {icon:'📅',label:'План дня',action:'navigate(\'plantoday\');closeLiveControl()'},
    {icon:'🗓',label:'План недели',action:'navigate(\'planweek\');closeLiveControl()'},
    {icon:'📆',label:'План месяца',action:'navigate(\'planmonth\');closeLiveControl()'},
    {icon:'🎓',label:'План обучения',action:'navigate(\'learnplan\');closeLiveControl()'},
    ...
    {icon:'🌿',label:'Восстановление',action:'navigate(\'recovery\');closeLiveControl()'},
    {icon:'🌐',label:'Домены',action:'navigate(\'domains\');closeLiveControl()'},
    {icon:'📊',label:'Статистика',action:'navigate(\'stats\');closeLiveControl()'},
    {icon:'🔗',label:'Интеграции',action:'navigate(\'integrations\');closeLiveControl()'},
...
function renderDashboard(){
  ...
  html+='<div class="card"><h2>👁 Зрение — быстро</h2>';
  html+='<div class="group-grid">';
  html+='<div class="group-item" onclick="startEyeExercise(\'v_2020\')"><div class="group-item-icon">⏱</div><div class="group-item-label">20-20-20</div></div>';
  html+='<div class="group-item" onclick="startEyeExercise(\'v_palm\')"><div class="group-item-icon">✋</div><div class="group-item-label">Пальминг</div></div>';
  html+='<div class="group-item" onclick="navigate(\'visionex\')"><div class="group-item-icon">🤸</div><div class="group-item-label">Все упр.</div></div>';
  html+='</div></div>';
...
function renderWisdom(){
  var w=(typeof getTodayWisdom==='function')?getTodayWisdom():{text:'Начни.',author:'Неизвестный',action:'Сделай шаг',apply:'Действие'};
...
function applyWisdom(){
  var w=(typeof getTodayWisdom==='function')?getTodayWisdom():{action:'Сделай шаг',apply:'Действие'};
  ...
  save();haptic('success');
  toast('✓ Мудрость внедрена в задачи','success');
  navigate('tasks');
}
...
function renderLearnPlanCard(){
  ...
  html+='<div class="row-between mb-3"><div style="font-size:16px;font-weight:800;">🎓 План обучения на сегодня</div>';
...
function generateLearnPlan(period){
  ...
  levels.forEach(function(level){
    level.modules.forEach(function(mod){
      mod.lessons.forEach(function(lesson,i){
        ...
        if(!state.levelProgress[key]&&items.length<8){
  ...
  (window.COURSES_LIBRARY||[]).forEach(function(c){
    ...
    if(completed<(c.lessons||[]).length&&items.length<12){
      items.push({id:'course_'+c.id,title:c.title,type:'Курс · '+c.category,icon:c.emoji,minutes:20,done:false,action:'course',courseId:c.id});
    }
  });
...
function renderLearning(){
  ...
  var engTotal=(window.__ENGLISH_125?window.__ENGLISH_125.length:125);
  ...
  var psychTotal=(window.PSYCHOLOGY_TOPICS||[]).length;
  var hormTotal=(window.HORMONES||[]).length;
  var wealthTotal=(window.WEALTH_MODULES||[]).length;
  ...
  html+='<div class="search-bar"><span style="color:var(--text-3);">🔍</span><input type="search" placeholder="Найти урок..." value="'+esc(learnSearchText)+'" oninput="learnSearchText=this.value;renderLearningSearch()"/></div>';
  ...
  html+='<div class="compact-item" onclick="navigate(\'psychology\')"><span class="compact-icon">🧠</span><span>Психология '+psychTotal+'</span></div>';
  html+='<div class="compact-item" onclick="navigate(\'hormones\')"><span class="compact-icon">🧬</span><span>Гормоны '+hormTotal+'</span></div>';
  html+='<div class="compact-item" onclick="navigate(\'wealth\')"><span class="compact-icon">💰</span><span>Богатство '+wealthTotal+'</span></div>';
...
function renderLearningSearch(){
  ...
  var results=[];
  (window.LEARNING_LEVELS||[]).forEach(function(l){
    l.modules.forEach(function(m){
      m.lessons.forEach(function(lesson,idx){
        if(lesson.title.toLowerCase().indexOf(q)>=0)results.push({icon:l.emoji,title:lesson.title,sub:l.title+' · '+m.title,onclick:'openLevelLessonFromSearch(\''+l.id+'\',\''+m.id+'\','+idx+')'});
      });
    });
  });
  (window.COURSES_LIBRARY||[]).forEach(function(c){
    if(c.title.toLowerCase().indexOf(q)>=0)results.push({icon:c.emoji,title:c.title,sub:'Курс · '+c.category,onclick:'openCourse(\''+c.id+'\')'});
  });
  (window.__SKILLS_LIBRARY||[]).forEach(function(s){
    if(s.title.toLowerCase().indexOf(q)>=0)results.push({icon:s.emoji,title:s.title,sub:'Навык',onclick:'openSkill(\''+s.id+'\')'});
  });
...
function openLesson(idx){
  ...
  if(lesson.test)html+='<div class="lesson-section test"><div class="lesson-section-title">❓ Проверь</div><div class="lesson-content">'+esc(lesson.test)+'</div></div>';
  html+='</div>';
...
function openStep(idx){
  ...
  if(!isDone)html+='<button class="btn btn-primary btn-block mt-3" onclick="completeStep('+idx+')">✓ Отметить</button>';
...
function completeStep(idx){
  ...
  if(userPath.completedSteps.indexOf(idx)<0){
    ...
    state.xp=(state.xp||0)+25;
    save();toast('✓ Шаг выполнен! +25 XP','success');haptic('success');
...
function openCourse(id){
  var c=(window.COURSES_LIBRARY||[]).find(function(x){return x.id===id});if(!c)return;
...
function openCourseLesson(courseId,idx){
  var c=(window.COURSES_LIBRARY||[]).find(function(x){return x.id===courseId});if(!c)return;
  ...
  if(lesson.example)html+='<div class="card"><div class="lesson-section example"><div class="lesson-section-title">💡 Пример</div><div class="lesson-content">'+esc(lesson.example)+'</div></div></div>';
  ...
  if(lesson.test)html+='<div class="card"><div class="lesson-section test"><div class="lesson-section-title">❓ Проверь</div><div class="lesson-content">'+esc(lesson.test)+'</div></div></div>';
...
function completeCourseLesson(courseId,idx){
  ...
  if(userCourse.completedLessons.indexOf(idx)<0){
    ...
    state.xp=(state.xp||0)+20;
    save();toast('✓ Урок изучен! +20 XP','success');haptic('success');
...
function renderVision(){
  ...
  html+='<button class="btn btn-ghost btn-block mt-2" onclick="navigate(\'visionex\')">Все упражнения ('+ex.length+')</button></div>';
  html+='<div class="card"><h2>💡 Советы для глаз</h2>';
  var tips=(window.SCREEN_TIPS||[]).filter(function(t){return t.category.indexOf('Зрение')>=0});
  tips.slice(0,4).forEach(function(t){
    html+='<div style="background:var(--glass-2);border-radius:14px;padding:12px;margin-bottom:8px;"><div style="font-weight:700;margin-bottom:4px;">'+esc(t.title)+'</div><div class="footnote text-secondary">'+esc(t.desc)+'</div><div class="footnote text-tertiary mt-1">✨ '+esc(t.effect)+'</div></div>';
  });
  html+='<button class="btn btn-ghost btn-block mt-2" onclick="navigate(\'visiontips\')">Все советы</button></div>';
...
function startEyeExercise(id){
  var all=window.VISION_EXERCISES_DETAILED||window.VISION_EXERCISES||[];
  var ex=all.find(function(x){return x.id===id});
  if(!ex)return;
  ...
  if(ex.subtitle)html+='<div class="footnote text-secondary" style="margin-top:4px;">'+esc(ex.subtitle)+'</div>';
  html+='<div class="footnote text-secondary">'+ex.duration+' · '+ex.benefit+'</div>';
  if(ex.level||ex.frequency){html+='<div class="row" style="justify-content:center;margin-top:8px;gap:6px;">';if(ex.level)html+='<span class="badge">'+ex.level+'</span>';if(ex.frequency)html+='<span class="badge">⏰ '+ex.frequency+'</span>';html+='</div>'}
  html+='</div>';
  if(ex.theory)html+='<div class="card"><div class="lesson-section theory"><div class="lesson-section-title">📚 Теория</div><div class="lesson-content">'+formatLesson(ex.theory)+'</div></div></div>';
  if(ex.science)html+='<div class="card"><div class="lesson-section science"><div class="lesson-section-title">🔬 Наука</div><div class="lesson-content">'+formatLesson(ex.science)+'</div></div></div>';
  if(ex.how)html+='<div class="card"><div class="lesson-section how"><div class="lesson-section-title">🎯 Как делать</div><div class="lesson-content">'+formatLesson(ex.how)+'</div></div></div>';
  if(ex.mistakes)html+='<div class="card"><div class="lesson-section mistakes"><div class="lesson-section-title">⚠️ Частые ошибки</div><div class="lesson-content">'+formatLesson(ex.mistakes)+'</div></div></div>';
  if(ex.effect)html+='<div class="card"><div class="lesson-section effect"><div class="lesson-section-title">💎 Эффект</div><div class="lesson-content">'+formatLesson(ex.effect)+'</div></div></div>';
  if(ex.tips)html+='<div class="insight-card"><div class="insight-title">💡 Лайфхаки</div><div class="insight-text">'+formatLesson(ex.tips)+'</div></div>';
...
function completeEyeExercise(){
  ...
  else if(currentPage==='visionex')renderVisionExercises();
  else renderDashboard();
}
...
function renderPlanning(){
  ...
  html+='</div>';
  html+='<div class="card"><h2>⏰ Когда планировать</h2>';
  html+='<div class="list-row"><div class="list-icon">🌙</div><div class="list-body"><div class="list-title">Вечер предыдущего дня</div><div class="list-subtitle">План дня на завтра</div></div></div>';
  html+='<div class="list-row"><div class="list-icon">🗓</div><div class="list-body"><div class="list-title">Воскресенье</div><div class="list-subtitle">План недели</div></div></div>';
  html+='<div class="list-row"><div class="list-icon">📆</div><div class="list-body"><div class="list-title">29-30 числа</div><div class="list-subtitle">План месяца</div></div></div>';
  html+='</div></div>';
  document.getElementById('app').innerHTML=html;
}
function renderPlanToday(){
  ...
  if(plan){
    ...
    (plan.tasks||[]).forEach(function(t,i){
      html+='<div class="task-item '+(t.done?'completed':'')+'" onclick="togglePlanTask(\'day\',\''+date+'\','+i+')"><div class="task-checkbox '+(t.done?'checked':'')+'">'+(t.done?'✓':'')+'</div><div class="task-content"><div class="task-title">'+esc(t.title)+'</div><div class="task-meta">'+(t.time||'—')+'</div></div></div>';
    });
    html+='</div>';
  }
  html+='<button class="btn btn-primary btn-block" onclick="createDayPlan(\''+date+'\')">'+(plan?'Обновить':'Создать')+' план</button>';
  html+='<div class="card" style="margin-top:12px;"><h2>📓 Шаблон из Obsidian</h2><div class="footnote text-secondary">Скопируй шаблон:</div><pre style="font-size:11px;background:var(--glass-2);padding:12px;border-radius:10px;overflow-x:auto;margin-top:8px;"># План на '+date+'\n\n## Задачи\n- [ ] Задача 1 @09:00\n- [ ] Задача 2 @14:00\n\n## Заметки\n_</pre></div>';
  html+='</div>';
  document.getElementById('app').innerHTML=html;
}
function createDayPlan(date){
  ...
  var tasks=taskTitles.split(',').map(function(t){return t.trim()}).filter(function(t){return t}).map(function(t){
    var parts=t.split('@');
    return{title:parts[0].trim(),time:parts[1]?parts[1].trim():'',done:false};
  });
  plan.tasks=tasks;
  state.dayPlans[date]=plan;
  save();toast('✓ План сохранён','success');
  renderPlanToday();
}
function togglePlanTask(type,date,idx){
  ...
  save();
  if(type==='day')renderPlanToday();
  else if(type==='week')renderPlanWeek();
  else renderPlanMonth();
}
function renderPlanWeek(){
  ...
  html+='<button class="btn btn-primary btn-block" onclick="createWeekPlan(\''+weekKey+'\')">'+(plan?'Обновить':'Создать')+' план недели</button>';
  html+='<div class="card" style="margin-top:12px;"><h2>📓 Шаблон из Obsidian</h2><pre style="font-size:11px;background:var(--glass-2);padding:12px;border-radius:10px;overflow-x:auto;margin-top:8px;"># План недели\n\n## Цели\n- [ ] Цель 1\n- [ ] Цель 2\n\n## Задачи\n- [ ] Задача 1\n- [ ] Задача 2</pre></div>';
  html+='</div>';
  document.getElementById('app').innerHTML=html;
}
function createWeekPlan(weekKey){
  var plan=state.weekPlans[weekKey]||{tasks:[]};
  var taskTitles=prompt('Задачи недели (через запятую):',plan.tasks.map(function(t){return t.title}).join(', '));
  if(taskTitles===null)return;
  ...
  save();toast('✓ План недели сохранён','success');
  renderPlanWeek();
}
function renderPlanMonth(){
  ...
  html+='<button class="btn btn-primary btn-block" onclick="createMonthPlan(\''+monthKey+'\')">'+(plan?'Обновить':'Создать')+' план месяца</button>';
  html+='<div class="card" style="margin-top:12px;"><h2>📓 Шаблон из Obsidian</h2><pre style="font-size:11px;background:var(--glass-2);padding:12px;border-radius:10px;overflow-x:auto;margin-top:8px;"># План месяца\n\n## Цели\n- [ ] Цель 1\n- [ ] Цель 2\n\n## Задачи\n- [ ] Задача 1</pre></div>';
  html+='</div>';
  document.getElementById('app').innerHTML=html;
}
function createMonthPlan(monthKey){
  var plan=state.monthPlans[monthKey]||{tasks:[]};
  var taskTitles=prompt('Задачи месяца (через запятую):',plan.tasks.map(function(t){return t.title}).join(', '));
  if(taskTitles===null)return;
  plan.tasks=taskTitles.split(',').map(function(t){return{title:t.trim(),done:false}}).filter(function(t){return t.title});
  state.monthPlans[monthKey]=plan;
  save();toast('✓ План месяца сохранён','success');
  renderPlanMonth();
}
function getWeekKey(){
  var d=new Date();
...
function renderObsidian(){
  ...
  html+='<button class="btn btn-primary btn-block mt-3" onclick="saveObsidian()">💾 Сохранить</button>';
  if(o.connected)html+='<button class="btn btn-ghost btn-block mt-2" onclick="syncObsidian()">🔄 Синхронизировать</button>';
  html+='</div>';
  html+='<div class="card"><h2>📖 Как подключить</h2>';
  html+='<ol style="padding-left:20px;line-height:1.8;font-size:13px;">';
  html+='<li>Открой Obsidian</li>';
  html+='<li>Настройки → Сторонние плагины → Включить</li>';
  html+='<li>Найди <b>Local REST API</b></li>';
  html+='<li>Установить → Включить</li>';
  html+='<li>Скопируй API Key из настроек плагина</li>';
  html+='<li>Вставь сюда + название vault</li>';
  html+='<li>Готово! Заметки синхронизируются</li>';
  html+='</ol></div>';
  html+='<div class="card"><h2>📝 Шаблоны заметок</h2>';
  html+='<div class="footnote text-secondary mb-2">Создай в Obsidian заметки:</div>';
  html+='<div class="group-card"><div class="group-title">📅 План дня</div><pre style="font-size:11px;overflow-x:auto;"># План на {{date}}\n\n## Задачи\n- [ ] Задача @09:00 #работа\n- [ ] Задача @14:00 #учёба</pre></div>';
  html+='<div class="group-card"><div class="group-title">🗓 План недели</div><pre style="font-size:11px;overflow-x:auto;"># План недели {{date}}\n\n## Цели\n- [ ] Цель 1\n\n## Задачи\n- [ ] Задача 1</pre></div>';
  html+='<div class="group-card"><div class="group-title">📆 План месяца</div><pre style="font-size:11px;overflow-x:auto;"># План месяца {{date}}\n\n## Цели\n- [ ] Цель 1</pre></div>';
  html+='</div></div>';
  document.getElementById('app').innerHTML=html;
}
function saveObsidian(){
  ...
  save();toast('✓ Obsidian сохранён','success');
  renderObsidian();
}
...
function renderIntegrations(){
  ...
  html+='<div class="list-row" onclick="navigate(\'gcal\')"><div class="list-icon">📅</div><div class="list-body"><div class="list-title">Google Calendar</div><div class="list-subtitle">Календарь событий</div></div><div class="list-chevron">›</div></div>';
...
function saveGemini(){
  ...
  save();toast('✓ Gemini сохранён','success');
  renderIntegrations();
}
...
function openWatchReflection(title,type){
  ...
  openSheet('Рефлексия: '+title,html);
}
function saveWatchReflection(title,type){
  var learn=(document.getElementById('reflection-learn')||{}).value||'';
  var main=(document.getElementById('reflection-main')||{}).value||'';
  var rating=parseInt((document.getElementById('reflection-rating')||{}).value)||8;
  if(!state.watched)state.watched=[];
  state.watched.push({id:uid(),title:title,type:type,learn:learn,main:main,rating:rating,date:today()});
  state.xp=(state.xp||0)+20;
...
function renderMovies(){
  ...
  lib.forEach(function(m){
    ...
    if(!watched)html+='<button class="btn btn-primary btn-sm mt-2" onclick="openWatchReflection(\''+esc(m.title).replace(/'/g,"\\'")+'\',\'movie\')">✓ Смотрел + рефлексия</button>';
    else html+='<div class="badge badge-success mt-2">✓ Просмотрено</div>';
    html+='</div></div>';
  });
  html+='</div>';
  document.getElementById('app').innerHTML=html;
...
function openDetoxDay(day){
  ...
  html+='<div class="footnote text-secondary">'+esc(d.phase)+'</div>';
  html+='</div>';
  html+='<div style="text-align:center;margin-bottom:16px;">';
  if(d.emoji)html+='<div style="font-size:48px;margin-bottom:8px;">'+d.emoji+'</div>';
  html+='<div style="font-size:22px;font-weight:800;">'+esc(d.title)+'</div>';
  if(d.subtitle)html+='<div class="footnote text-secondary" style="margin-top:4px;">'+esc(d.subtitle)+'</div>';
  html+='</div>';
  ...
  if(d.xp)html+='<div class="footnote text-tertiary" style="text-align:center;margin:14px 0;">Награда: +'+d.xp+' XP</div>';
...
function renderSettings(){
  ...
  html+='<div class="footnote text-tertiary" style="text-align:center;margin-top:24px;">Life OS v33</div></div>';
  document.getElementById('app').innerHTML=html;
}
...
function exportDB(){
  ...
  a.href=url;a.download='life-os-'+today()+'.json';
...
function renderMore(){
  var groups=[
    {title:'🎓 Обучение',items:[
      ...
      {key:'courses',emoji:'📖',label:'Курсы'},
      {key:'paths',emoji:'🗺',label:'Пути'},
      {key:'methods',emoji:'🎯',label:'Методики'},
      {key:'psychology',emoji:'🧠',label:'Психология'},
      {key:'hormones',emoji:'🧬',label:'Гормоны'},
      {key:'wealth',emoji:'💰',label:'Богатство/Успех/Счастье'}
    ]},
    ...
    {title:'👁 Зрение',items:[
      ...
      {key:'visionex',emoji:'🤸',label:'Упражнения'},
...
function skipDailySurveyStep(){currentDailySurveyStep=(currentDailySurveyStep||0)+1;if(currentDailySurveyStep>=DAILY_SURVEY_QUESTIONS.length)finishDailySurvey();else renderDailySurvey()}
function saveDailySurveyAnswer(){var q=DAILY_SURVEY_QUESTIONS[currentDailySurveyStep];if(!q)return;if(q.type==='number'){var el=document.getElementById('dn');if(el)currentDailySurveyAnswers[q.id]=parseFloat(el.value)}else if(q.type==='text'){var el2=document.getElementById('dt');if(el2)currentDailySurveyAnswers[q.id]=el2.value}}
function finishDailySurvey(){
  ...
  if(answers.screenMinutes!=null){if(!state.screenHistory)state.screenHistory={};state.screenHistory[y]=parseFloat(answers.screenMinutes)}
...
function buildTodayPlan(answers){
  ...
  if(mood<6||stress>6){plan.items.push({time:'день',title:'Медитация 10 мин',desc:'Стресс',icon:'🧘'});plan.items.push({time:'день',title:'Дыхание 4-7-8',desc:'Успокоение',icon:'🌬'})}
...
function getAdaptiveLoad(){
  ...
  var load=100;
  if(sleep<6)load-=20;if(sleep<7)load-=10;if(water<4)load-=10;if(mood<5)load-=15;if(screen>300)load-=10;if(screen>420)load-=10;
...
function showWelcome(){
  ...
  overlay.innerHTML='<div class="welcome-logo">🧠</div><div class="welcome-title">Life OS</div><div class="welcome-sub">Нейроэкосистема жизни. Обучение, планирование, зрение, AI, интеграции.</div><button class="btn btn-primary btn-block" style="max-width:340px;" onclick="startOnboarding()">Начать</button>';
  document.body.appendChild(overlay);
}
...
function init(){
  try{
    ...
    ['customHabits','customGoals','customNotes','journalEntries','customWater','customMood',
     ...
     'watchlist','watched','customResources','paths','courses','eyeExercises'].forEach(function(k){
      if(!state[k])state[k]=[];
    });
...
function attachRipple(){
  document.addEventListener('pointerdown',function(e){
    var target=e.target.closest('.btn,.list-row,.quick-tab,.tab-item,.task-item,.card,.icon-btn,.avatar-btn,.segmented-item,.level-card,.module-card,.lesson-row,.domain-card,.method-card,.course-card,.path-step,.survey-option,.ent-card,.habit-row,.group-item,.live-panel-item,.detox-day-card,.compact-item,.resource-card,.matrix-quadrant,.cal-nav-btn,.cal-today-btn,.cal-view-btn,.cal-month-cell,.cal-week-day,.cal-event-card,.cal-color-btn,.pill-btn,.pill-icon-btn');
    if(!target)return;
