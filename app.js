...
try{
  ...
  if(raw){
    ...
    ['levelProgress','englishProgress','skillsProgress','domainScores','metrics',
     'challengeProgress','screenHistory','customSleep','dailySurveys'].forEach(function(k){
      if(!state[k])state[k]={};
    });
...
function save(){
  try{state.settings.updatedAt=nowISO();localStorage.setItem(STORAGE_KEY,JSON.stringify(state))}
...
function startEffects(){
  ...
  try{
    ...
    var theme=(window.THEMES||[]).find(function(t){return t.id===state.settings.theme})||{effects:'stars'};
    var effect=theme.effects||'stars';
    function cnt(base){return Math.round(base*(state.settings.effectsIntensity||1)*2.5)}
    if(effect==='stars'||effect==='none'){
      ...
      for(var j=0;j<cnt(25);j++){var d=document.createElement('div');d.className='effect-drop';d.style.left=Math.random()*100+'%';d.style.animationDuration=((Math.random()*1.5+1.5))+'s';d.style.animationDelay=Math.random()*5+'s';d.style.height=(Math.random()*20+12)+'px';overlay.appendChild(d)}
    }else if(effect==='petals'||effect==='hearts'){
      for(var k=0;k<cnt(12);k++){var p=document.createElement('div');p.className='effect-petal';p.textContent=effect==='hearts'?'❤':'🌸';p.style.left=Math.random()*100+'%';p.style.animationDuration=((Math.random()*8+10))+'s';p.style.animationDelay=Math.random()*10+'s';overlay.appendChild(p)}
      ...
      for(var n=0;n<cnt(18);n++){var sn=document.createElement('div');sn.className='effect-snowflake';sn.textContent='❄';sn.style.left=Math.random()*100+'%';sn.style.animationDuration=((Math.random()*6+8))+'s';sn.style.animationDelay=Math.random()*10+'s';overlay.appendChild(sn)}
...
function navigate(page){
  ...
  if(typeof fn!=='function'){
    main.innerHTML='<div class="page">'+backBtn('dashboard')+'<div class="title-xl">🚧 '+page+'</div><div class="card"><div class="empty"><div class="empty-icon">🚧</div><div class="empty-title">В разработке</div></div></div></div>';
    return;
  }
  try{fn()}catch(e){
...
function renderDashboard(){
  ...
  (window.WORK_MODES||[]).forEach(function(m){
    ...
    html+='<div class="mode-card '+(active?'active':'')+'" onclick="setWorkMode(\''+m.id+'\')"><div style="font-size:20px;">'+m.emoji+'</div><div style="font-size:13px;font-weight:800;">'+m.name+'</div><div class="footnote text-secondary">'+m.desc+'</div></div>';
  });
  html+='</div>';
...
function getEisenhowerPriority(t){var q=getEisenhowerQuadrant(t);return q==='q1'?1:q==='q2'?2:q==='q3'?3:4}
function taskRow(t){
...
function completeLesson(){
  ...
  var xpAdd=(typeof applyLearningBonus==='function')?applyLearningBonus(10):25;
...
function completeStep(idx){
  ...
  if(userPath.completedSteps.indexOf(idx)<0){
    ...
    state.xp=(state.xp||0)+xpAdd;
    save();toast('✓ Шаг выполнен! +'+xpAdd+' XP','success');haptic('success');
...
function completeCourseLesson(courseId,idx){
  ...
  if(userCourse.completedLessons.indexOf(idx)<0){
    ...
    var xpAdd=(typeof applyLearningBonus==='function')?applyLearningBonus(20):50;
    state.xp=(state.xp||0)+xpAdd;
    save();toast('✓ Урок изучен! +'+xpAdd+' XP','success');haptic('success');
...
function renderMatrix(){
  ...
  state.tasks.forEach(function(t){if(t.status==='completed')return;matrix[getEisenhowerQuadrant(t)].push(t)});
...
function renderSettings(){
  ...
  html+='<div class="footnote text-tertiary" style="text-align:center;margin-top:24px;">AI Health v33</div></div>';
  document.getElementById('app').innerHTML=html;
}
...
function renderIntegrations(){
  ...
  html+='<div class="card"><h2>✨ Gemini AI</h2><div class="field"><label class="field-label">API Key</label><input type="password" id="gem-key" value="'+esc(state.settings.apiKey||'')+'"/></div><button class="btn btn-primary btn-block" onclick="saveGemini()">💾 Сохранить</button></div>';
  html+='</div>';
  document.getElementById('app').innerHTML=html;
}
function saveGemini(){
  state.settings.apiKey=(document.getElementById('gem-key')||{}).value||'';
...
function showWelcome(){
  ...
  overlay.innerHTML='<div class="welcome-logo">🧠</div><div class="welcome-title">AI Health v33</div><div class="welcome-sub">One UI 9 · Обучение · Опросы · AI.</div><button class="btn btn-primary btn-block" style="max-width:340px;" onclick="startOnboarding()">Начать</button>';
  document.body.appendChild(overlay);
}
