...
function openLiveControl(){
  ...
  var items=[
    ...
    {icon:'🇬🇧',label:'English 125',action:'navigate(\'english\');closeLiveControl()'},
...
function renderDashboard(){
  ...
  html+='<div class="card" style="background:linear-gradient(135deg,rgba(255,107,107,.15),rgba(255,169,64,.1));border-color:rgba(255,107,107,.3);"><h2>📱 30-дневный детокс</h2><div class="footnote text-secondary" style="margin-bottom:12px;">Пройдено: '+detoxCompleted+'/30</div><button class="btn btn-primary btn-block" onclick="navigate(\'detoxcourse\')">Открыть</button></div>';
  ...
  if(!state.profile.surveyDone){
    html+='<div class="card" style="background:linear-gradient(135deg,rgba(255,169,64,.15),rgba(255,107,107,.1));border-color:rgba(255,169,64,.3);"><h2>📋 Заполни профиль</h2><div class="footnote text-secondary" style="margin-bottom:12px;">26 вопросов → персональный план</div><button class="btn btn-primary btn-block" onclick="startSurvey()">Начать</button></div>';
  } else {
    html+='<div class="card" style="background:linear-gradient(135deg,rgba(61,220,151,.15),rgba(91,158,255,.1));border-color:rgba(61,220,151,.3);"><h2>🎯 Персональный план</h2><button class="btn btn-primary btn-block" onclick="navigate(\'plan\')">Открыть</button></div>';
  }
...
function renderSettings(){
  ...
  html+='<div class="footnote text-tertiary" style="text-align:center;margin-top:36px;">AI Health v31 · Vision UI</div></div>';
  document.getElementById('app').innerHTML=html;
}
...
function exportDB(){
  ...
  a.href=url;a.download='ai-health-v31-'+today()+'.json';
...
function showWelcome(){
  ...
  overlay.innerHTML='<div class="welcome-logo">🧠</div><div class="welcome-title">AI Health v31</div><div class="welcome-sub">5 уровней · 122 навыка · 125 уроков English · 45+ тем · 30-дневный детокс · Досуг · AI.</div><button class="btn btn-primary btn-block" style="max-width:340px;" onclick="startOnboarding()">Начать</button>';
  document.body.appendChild(overlay);
}
