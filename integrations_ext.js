...
async function syncNotesToObsidian(){
  ...
  for(var i=0;i<notes.length;i++){
    var n=notes[i];
    var content='# '+(n.title||'Без названия')+'\n\nДата: '+n.created_at+'\n\n'+(n.content||'');
    var path=OBSIDIAN_CONFIG.defaultFolder+'/Notes/'+sanitizeFilename(n.title||'note')+'.md';
...
async function syncLearningToObsidian(){
  ...
  if(state.skillsProgress){
    content+='## 💎 Навыки\n\n';
    content+='Изучено: '+Object.keys(state.skillsProgress).length+'\n\n';
  }
...
async function syncJournalToObsidian(){
  ...
  for(var i=0;i<journal.length;i++){
    var j=journal[i];
    var dateStr=j.date||new Date().toISOString().slice(0,10);
    var content='# '+dateStr+'\n\n'+(j.content||'');
    var path=OBSIDIAN_CONFIG.defaultFolder+'/Journal/'+dateStr+'.md';
    if(await OBSIDIAN_API.saveNote(path,content))ok++;
  }
  return ok;
}
...
function saveObsidianConfig(){
  try{localStorage.setItem('obsidian_config',JSON.stringify(OBSIDIAN_CONFIG))}catch(e){}
}
...
var GCAL_API={
  ...
  isAuthorized(){return GCAL_CONFIG.accessToken&&GCAL_CONFIG.expiresAt>Date.now()},
...
function saveGCalConfig(){try{localStorage.setItem('gcal_config',JSON.stringify(GCAL_CONFIG))}catch(e){}}
function loadGCalConfig(){try{var data=JSON.parse(localStorage.getItem('gcal_config')||'null');if(data)Object.assign(GCAL_CONFIG,data)}catch(e){}}
  ...
  async createTask(content,priority,dueString){
    ...
    try{
      var resp=await fetch('https://api.todoist.com/rest/v2/tasks',{
        ...
        body:JSON.stringify({content:content,priority:priority||1,due_string:dueString||'today'})
      });
      return await resp.json();
    }catch(e){return null}
  },
...
async function callGeminiExtended(prompt,history){
  ...
  try{
    ...
    var resp=await fetch('https://generativelanguage.googleapis.com/v1beta/models/'+GEMINI_CONFIG.model+':generateContent?key='+GEMINI_CONFIG.apiKey,{
      ...
      body:JSON.stringify({
        contents:contents,
        generationConfig:{temperature:GEMINI_CONFIG.temperature,maxOutputTokens:GEMINI_CONFIG.maxTokens}
      })
    });
    var data=await resp.json();
...
window.GCAL_API=GCAL_API;
window.OBSIDIAN_API=OBSIDIAN_API;
...
window.NOTION_API=NOTION_API;
window.TODOIST_API=TODOIST_API;
window.initIntegrationsExt=initIntegrationsExt;