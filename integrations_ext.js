'use strict';
/* INTEGRATIONS EXTENDED — OBSIDIAN, GOOGLE CALENDAR, NOTION, TODOIST, GEMINI, OPENAI */
...
async function syncJournalToObsidian(){
  ...
  for(var i=0;i<journal.length;i++){
    var j=journal[i];
    var content='# '+j.date+'\n\n'+j.content;
    var path=OBSIDIAN_CONFIG.defaultFolder+'/Journal/'+j.date+'.md';
    if(await OBSIDIAN_API.saveNote(path,content))ok++;
  }
  return ok;
}
...
async function syncAllToObsidian(){
  if(!OBSIDIAN_CONFIG.apiKey){
    toast('Obsidian API key не настроен','error');
    return false;
  }
  ...
  toast('Синхронизировано: '+okCount+'/4','success');
  return true;
}
...
async function importFromObsidian(folder){
  ...
  if(!files||files.length===0){
    toast('Файлы не найдены','warning');
    return 0;
  }
  var imported=0;
  ...
  if(imported>0)save();
  toast('Импортировано: '+imported,'success');
  return imported;
}
...
var GCAL_API={
  async authorize(){
    if(!GCAL_CONFIG.clientId){
      toast('Client ID не настроен','error');
      return false;
    }
  ...
  checkTokenFromUrl(){
    if(window.location.hash){
      ...
      if(token){
        ...
        toast('Google Calendar подключён','success');
        return true;
      }
    }
    return false;
...
async function syncTasksToGCal(){
  if(!GCAL_API.isAuthorized()){
    toast('Сначала авторизуйся','error');
    return 0;
  }
  ...
  saveGCalConfig();
  toast('Синхронизировано: '+ok,'success');
  return ok;
}
...
async function createGCalEventFromTask(taskId){
  ...
  if(!t)return null;
  if(!t.due_date){
    toast('Нет даты у задачи','warning');
    return null;
  }
  var event={
    summary:t.title,
